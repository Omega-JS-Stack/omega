...
| [src/launch-omega-extension.js](../../packages/mcp-router/src/launch-omega-extension.js) | Runs the extension MCP server this package ships, beside it at `servers/omega-extension/index.js` |
| [servers/omega-extension/](../../packages/mcp-router/servers/omega-extension/) | That server: the MCP-to-WebSocket bridge to the OMEGA Companion extension, its tool list, and the upstream's bundled `config.json` |
...
| `omega-extension` | `node ${MCP_ROUTER_ROOT}/src/launch-omega-extension.js` | auto | The OMEGA companion extension's automation bridge. Needs nothing installed: the server ships here ([#927](https://github.com/Omega-JS-Stack/omega/issues/927)) |

Each ships with its tool schemas cached, so a fresh install lists tools without spawning anything.

`omega-extension` needs the extension itself at the other end of its socket, and that is the ONE thing to install: the OMEGA Companion on the Chrome Web Store for use, or, to develop it, `omega-omega/targets/extension/packaged/chrome/raw/` loaded unpacked at `chrome://extensions` (Developer mode on; `npx omega build` in that target writes it, and a brand still on a pre-[#867](https://github.com/Omega-JS-Stack/omega/issues/867) framework writes `packaged/chromium/raw/` under the old browser vocabulary). The extension is the brand's own target, not the router's and not the manager's: whichever side holds the socket, the port is 9876 and `OMEGA_EXTENSION_PORT` moves the server half of it.
...
- **The `omega-extension` upstream is only half a product without the extension.** The bridge ships here and starts anywhere, but the tools answer only once the OMEGA Companion is connected: from the Chrome Web Store, or loaded unpacked out of `omega-omega/targets/extension/packaged/chrome/raw/` (above). The store listing does not exist yet, so today the unpacked load is the only way in. Until [#927](https://github.com/Omega-JS-Stack/omega/issues/927) the server lived in `@omega.js/manager`'s `extension/` tree, which that package never publishes: the upstream could not start on any install but a monorepo checkout, and the sibling-package fallback hid it here.
