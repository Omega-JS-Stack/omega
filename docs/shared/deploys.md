...
- The other half is a trap worth knowing: the scaffolded backend `deploy` script is `npx omega setup && npx omega deploy`, which in custom mode ends in the framework's refusal. That is deliberate — it fails loudly, on the line that says what to put there, instead of quietly deploying nothing.
