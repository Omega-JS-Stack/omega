...
| the verbs' scaffold | Runs, minus the Firebase-only half ([#614](https://github.com/Omega-JS-Stack/omega/issues/614)) — see below |
...
The key inventory (owner, targets, generated-or-third-party, required, dev twin, description) lives in `@omega.js/config`'s env schema — the same list the manager mints from and the composer below delivers from: [docs/shared/config.md](../shared/config.md).
...
- **The layers, weakest first**: company `.env` ← brand `.env` ← target `.env`, resolved through `@omega.js/config`'s `resolveEnvChain` (the same walk the runtime cascade uses). Files only — never the shell, so a build produces the same artifact under any terminal. An empty value never claims a key.
- **The schema is the only filter.** A key rides down from the company/brand layers when an `ENV_SCHEMA` entry claims it (by name OR by pattern — the `OAUTH2_<PROVIDER>_CLIENT_ID`/`_SECRET` family composes), names `backend` in its `targets`, and sits in a file group. A desktop signing key stays out of the functions upload. The target layer passes through UNFILTERED: placing a key in the target's own `.env` is the targeting.
...
- **A deploy stage drops the `_DEV` rows** as before (`stageFunctions({ deploy: true })`).
...
Backend has no separate deploy PRECHECK: its network steps are inside the verb itself ([#675](https://github.com/Omega-JS-Stack/omega/issues/675)). `omega deploy` sets the Cloud Functions artifact cleanup policy and grants the public `allUsers` invoker on every HTTP function before it uploads — both idempotent, both skipped without a fuss when the project has nothing to set yet. There is nothing to opt out of, so the `--no-secrets` flag the other three frameworks honor is accepted and IGNORED here (parity of spelling, not of behavior).
...
All `npm install` calls in CLI commands (`npx omega i`, the target checks) route through the `safeInstall()` helper (`src/cli/utils/safe-install.js`). It prefixes `sfw` (Socket Firewall) when installed — blocking confirmed malware at the network level before packages reach disk. Falls back to plain npm if sfw isn't available. CI workflows install sfw globally and run `sfw npm install`/`sfw npm ci`. Installs will **fail if sfw detects confirmed malware** in any package in the dependency tree; non-critical CVEs and quality warnings pass through.
