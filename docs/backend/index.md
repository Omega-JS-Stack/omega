...
`npx omega <command>` (bins `omega`, `omg`, `mgr`, all the same context-aware dispatcher):
...
**The backend deploys through a workflow like every other target** ([#872](https://github.com/Omega-JS-Stack/omega/issues/872)): its template is `src/defaults/.github/workflows/deploy.yml`, composed into the brand root as `backend-deploy.yml` by the same `composeTargetWorkflows` the other three ride, on `workflow_dispatch` alone ([#923](https://github.com/Omega-JS-Stack/omega/issues/923)) and no push trigger. So `npx omega deploy` in `targets/backend` is a DISPATCH now, and the runner's job is: checkout (depth 1, nothing here reads git history), setup-node, the rendered firewall step, `sfw npm ci` (the brand lockfile as pushed, [#938](https://github.com/Omega-JS-Stack/omega/issues/938)), the target's `.env` written by a node one-liner that reads the generated `{{ envFileKeys }}` list out of the runner env and serializes the file through `@omega.js/config`'s own `serializeEnv` (resolved from the just-installed backend at `@omega.js/backend/dist/vendor/config/env.js`, since the config package is vendored, not a dependency): no shell ever touches a secret, so a value carrying a newline can neither split a line nor inject a key of its own. Then `service-account.json` written from `OMEGA_SERVICE_ACCOUNT_JSON`, the Firebase CLI installed through the firewall (`sfw npm install -g firebase-tools@15`: the verb shells out to a GLOBAL `firebase`, the developer's tool rather than a framework dependency, and a runner starts without it), `gcloud auth activate-service-account` against that same file (the public-invoker fix shells out to `gcloud`, which reads its own credential store and ignores `GOOGLE_APPLICATION_CREDENTIALS`, so without it every CI-deployed HTTP function answers 403; the project stays implicit, every call the deploy makes passes its own `--project`), then `node "${{ github.workspace }}/node_modules/@omega.js/backend/bin/omega" deploy --direct` with `GOOGLE_APPLICATION_CREDENTIALS` exported to that file. No step in that workflow runs `npx` at all, the version-log step included ([#877](https://github.com/Omega-JS-Stack/omega/issues/877)): a bare `npx omega` on a runner where the shared bin never got linked fetches a stranger's package from the registry and runs it with the secrets in env, so the workflow resolves no bin NAME and runs this framework's own file. The job sets no `NODE_ENV` either, since `NODE_ENV=production` is what left the bin unlinked, by making npm skip every devDependency the brand declares, and its install names only this target's workspace ([deploys.md](../shared/deploys.md)). `.firebaserc` never travels (it is gitignored), and nothing has to send it: `ensureTarget` re-derives it on the runner from the resolved `cloud.config.projectId`, which is the direction that rule has always run. `--direct` is the whole of today's local behavior and is unchanged: the artifact cleanup policy, the pack step for any linked framework, `firebase deploy`, the public-invoker fix. The brand-root fan-out and `omega pipeline` pass `--direct` to the backend the way they already do for web. The lane, the secrets delivery and the pack step: [deploys.md](../shared/deploys.md).
