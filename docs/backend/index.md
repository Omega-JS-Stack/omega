...
- [docs/paypal-sandbox-qa.md](../../packages/backend/docs/paypal-sandbox-qa.md) — the live PayPal sandbox QA drive: standing fixtures (creds, webhook, the trial-free `hidden: true` `proof-press` product, the hand-made sandbox buyer — PayPal has no account-creation API), the subscription sale → refund sequence, and the gotchas
