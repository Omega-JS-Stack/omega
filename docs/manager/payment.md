...
interval + amount + trial, duplicates and stale plans deactivated, legacy products' active
plans deactivated so no new subscription lands on them).
...
**Credentials**: `STRIPE_SECRET_KEY`, `PAYPAL_CLIENT_SECRET`, `CHARGEBEE_API_KEY` — each
OPTIONAL (`gates: false`), so a missing Stripe key never gates a PayPal brand and each entry's
"Disable permanently" writes only its own provider's `false`. `OMEGA_WEBHOOK_KEY` is OMEGA's
own and is minted, not asked for. `--provider=stripe|paypal|chargebee` narrows a run to one
