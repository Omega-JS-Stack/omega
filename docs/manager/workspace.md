...
| `scripts` | Root `package.json` scripts say `omega` (a legacy omega-manager brand is healed) and a `deploy` script exists; each framework target's missing scripts fill from its framework's `projectScripts` declaration (the onboard→dev cycle break, #675 — custom targets and a custom-server backend own theirs). The walk only ever FILLS; the framework's own ensureTarget syncs those keys unconditionally on every verb run and wins from then on. Script VALUES only; no other key is touched. |
...
| `env-order` | Rewrites the brand (and company) `.env` into the canonical group order. |
...
shell > brand `.env` > company `.env` — never the brand file alone. A company-managed brand
therefore never shadows its company's value with a freshly minted one. A minted value is
...
target's resolved config, so a per-surface key is judged where the services write it.
