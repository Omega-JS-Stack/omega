...
| 2b — Ads company-mode e2e | The **cross-brand ads proof** (Paperloom's backend emulator serves seeded `ads` inventory — HTML unit / scoring / 204 / fail-closed redirect / whitelist scoping — and The Daily Build consumes as `source: 'company'`: real `@omega.js/config` compose → real `@omega.js/client` resolution → the consumer-built URL fetched against the parent stack) | `npm run test:ads` at the root | Every checkpoint touching the ads chain; `OMEGA_SKIP_E2E=1` to skip |
