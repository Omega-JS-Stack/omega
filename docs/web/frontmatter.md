...
| `catalog` | this page's entry in `pages.json`, the CATALOG the site's own search box reads: `catalog.include: false` keeps the page out of it, `catalog.category` names the section the entry is filed under (a post falls back to `post.category`). Page-only, with no site-wide default: omega.json5 has no catalog section, and its `search` section is Search Console, a different thing that happens to sit near the same word ([#858](https://github.com/Omega-JS-Stack/omega/issues/858), Ian 2026-09-13). It is a SEARCH-BOX opt-out and nothing else: the page stays indexable and stays in the sitemap. The old spelling `search.include` / `search.category` is gone, and a page still writing `search:` fails the build like any bare config section |
| `translation` | `translation.include: true` / `false`, this page's own answer to the site's `translation.include` list ([#858](https://github.com/Omega-JS-Stack/omega/issues/858), Ian's same-name ruling): `true` translates a page the list left out (one post under an untranslated `/blog`), `false` takes one out that the list covers. The build stamps it on `<html data-omega-translate>` for the post-build pass ([docs/shared/translation.md](../shared/translation.md)). `include` is the ONLY key this bare block may carry: it is a config SECTION name, and anything else under it is the bare restate the `config:` rule refuses, which fails the build |
