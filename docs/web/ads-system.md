...
- **Every click destination is UTM-tagged automatically**, both lanes, through one tagger: `applyVertUtm` in `@omega.js/client`'s `modules/vert-document.js`. The promo lane tags its own `https://omegajs.dev` link in the browser (`utm_medium=omega-vert`, `utm_campaign=omega-promo`, `utm_content=<the slot's size preset>`); a served vert is tagged by the backend redirect route on the STORED link (`utm_medium=omega-vert`, `utm_campaign=<vert id>`), which is where the final destination is known.
- **`utm_source` is the HOST brand's `brand.id`** (Ian's ruling 2026-07-31), with the parent host as the fallback when no id is available. The promo lane reads the id straight off the client's own config; the house lane sends it to the backend as the serve URL's `brand` param, the serve route stamps it on the redirect URL it builds, and the redirect route tags the stored link with it. Targeting is untouched: `?parent=<host>` still flows exactly as before (eligibility, postMessage origin, and the utm fallback), and only the tag's value changed. The backend accepts a `brand` value only when it matches `brand.id`'s own slug pattern, so nothing caller-supplied can write free text onto an advertiser's URL.
...
6. Company-mode proof on the playground (OMEGA Playground serves, a second in-repo brand consumes). (DONE — root `npm run test:verts` / scripts/e2e-verts-company.js; The Daily Build carries the standing `source: 'company'` + `company.url` config)
