...
| `redirect-map.js` | The browser half of the path-redirect map ([#442](https://github.com/Omega-JS-Stack/omega/issues/442)): applies the compiled `{ pattern, target }` entries the build inlines into the 404 page against a request path, first match wins. It parses NOTHING — the `:name` pattern engine is `packages/web/src/redirects.js`, and this file only ever runs the regex it was handed. DOM-free on purpose; `modules/redirect-map.js` is the half that reads the page and hops | `matchRedirect(entries, pathname)` |
