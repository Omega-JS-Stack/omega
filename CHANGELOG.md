...
### Added
- [#698](../../issues/698) — A Namecheap call refused for a non-whitelisted IP is walked, not warned: the domain service names the caller IP, gates on Enter to open the API access page, and rechecks the refused call until it passes. Non-interactive runs warn and continue.
- [#661](../../issues/661) — `/signin?authPrivateKey=<api.privateKey>` signs in from a URL that keeps working: the auth pages trade the key for a fresh custom token at `POST /omega/user/token` on every load, then strip it from the address bar. Custom tokens expire in an hour; a saved dock URL does not.
- [#693](../../issues/693) — The campaigns service creates SendGrid link branding for `emailurl.<domain>` when the account has none, writes its two CNAMEs into Cloudflare, waits for SendGrid validation, and flips the record to proxied in the same walk, so emailed links land on HTTPS with no console visit.
- [#687](../../issues/687) — `brands/naked-brand`: a bare fixture brand (brand.id, brand.name, one website target, demo cloud, nothing else) so a QA walkthrough sees every setup prompt fire from zero. Offline and test-only forever; nothing provisions real cloud resources from it.
- [#608](../../issues/608) — One setup contract for every manage service: a missing credential is asked for right then, with Provide / Skip this run / Disable permanently (`<service>.enabled: false`). Non-interactive runs never prompt — they print the exact keys and step aside.
...
### Removed
- [#636](../../issues/636) — The checkout's crypto pay button, its `providers.coinbase` reads and the `crypto → coinbase` provider mapping, with the config stub they depended on.
- [#636](../../issues/636) — `COINBASE_API_KEY`, its `payment.providers.coinbase` config stub and `APOLLO_API_KEY`: keys with no reader anywhere — there is no coinbase payment provider (the lane is stripe, paypal, chargebee, test) and no Apollo enrichment. A brand config still carrying the stub now says so in a validator warning.

### Fixed
- [#661](../../issues/661) — A key in the address bar never reaches Sentry: the failed private-key sign-in strips it before it captures, and the browser SDK scrubs `authPrivateKey` and `authCustomToken` out of navigation breadcrumbs and event request urls.
- [#699](../../issues/699) — A verb run outside any target refuses instead of scaffolding one there: the dispatcher's no-context fallback allows only onboard, help, version, cwd and logs, and every framework's ensure-target refuses a workspace root by name. `npx omega onboard` in a fresh clone still works.
...
- [#702](../../issues/702) — The user-delete route's internal sessions call drops `log: true`, so wonderful-fetch stops printing its configuration — the admin-key header included — into Cloud Logging. A source-scan test fails any options object pairing that flag with a credential header.
- [#701](../../issues/701) — A rate-limited OAuth signup says so: the returning redirect result resolves inside the return leg's own try, so a blocking-function rejection (`auth/error-code:-47`) shows its message and re-enables the form instead of escaping uncaught and leaving disabled fields and spinners.
- [#700](../../issues/700) — Signup stops waiting on the server: the web auth listener fires the `/omega/user/signup` POST without awaiting it, the legacy consent guard is deleted, and an account doc that is not written yet is the normal state — only a rules permission-denied signs the user out.
- [#692](../../issues/692) — The edge service reads SendGrid's domain-auth values live off the SendGrid API when building DNS records, so those records exist on any authenticated brand; unauthenticated brands get a printed skip. **Legacy brands: delete `edge.providers.cloudflare.dns.sendgrid` from omega.json5 — ignored now.**
- [#685](../../issues/685) — The client's `request.js` and package main drop their guarded `module.exports` tail: bundled to CJS, the tail ran last and clobbered the bundle's own exports. Pure ESM is the one mechanism now, and the web test harness imports the real module instead of a stub.
...
- [#667](../../issues/667) — An Internal OAuth consent screen STOPS the manage walk: Google offers no API write for the audience, so the run opens the console page on ENTER and polls until the flip lands. Skip warns; `cloud.consentAudience: false` records "never ask again". An org-less create failure names its cause.
- [#656](../../issues/656) — GA4 deduplicates `purchase` on `transaction_id`, so a money event now carries ONE CHARGE's id instead of the subscription's: the order id on a first purchase or trial start, the provider's charge id on a renewal, recovery or conversion, the reversed charge's id on a refund. Renewals stop being dropped.
- [#646](../../issues/646) — Links in transactional email land on HTTPS: `emailurl.<domain>` is created PROXIED once SendGrid reports the branded link `valid: true` (`GET /v3/whitelabel/links`), so Cloudflare terminates TLS instead of SendGrid serving no certificate. Until then the DNS ensure warns, naming the record. Legacy brands need the one-time flip.
- [#641](../../issues/641) — The OAuth2 providers log the provider, the uid and whether the token exchange succeeded, nothing else. All three used to write the exchange response (access, refresh and id tokens) into Cloud Logging in plain text, and the identity fetches printed their authorization headers. `auth:on-create` logs the uid alone.
- [#636](../../issues/636) — The schema declares what the services actually read and write back (`certificates`, both `domain` roles, the TikTok `appId`, `advertising.fallback`/`tags` and more), and the validator WARNS naming every undeclared key.
