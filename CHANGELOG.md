...
- [#603](../../issues/603) — `targets.<name>: { type: 'custom' }` declares a target no framework owns; its own `package.json` scripts define `start`, `build`, `test`, `deploy`, `clean`. Framework services skip it — env disperse and the workspace service do not.
...
- [#636](../../issues/636) — The schema declares what the services actually read and write back (`certificates`, both `domain` roles, the TikTok `appId`, `advertising.fallback`/`tags` and more), and the validator WARNS naming every undeclared key. Disperse now CLEARS: a composed key the brand root un-set is removed from the target `.env`.
