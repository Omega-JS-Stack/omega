...
### Changed
- [#363](../../issues/363) — Referrals are seeded on the dedicated `referrer` persona and nowhere else: premium-active keeps its subscription and its signed-in devices and carries no affiliate list, so every persona demonstrates exactly its own scenario. The referrer keeps its full four-referral history, ISO-dated exactly as a signup writes it.

### Fixed
- [#361](../../issues/361) — `omega deploy --direct` publishes a gh-pages PROJECT site instead of refusing it: no `brand.url` means the address is derived from the repo slug, the build mounts under the matching base path, and the CNAME step is skipped. The slug falls back to `GITHUB_REPOSITORY`, then the git remote.
- [#359](../../issues/359) — Language links on a site served under a URL path now point where the copy actually lives: the translation pass reads the base path off the build's stamp and composes prefix-then-lang (`/workkit/es/pricing`), matching exclusions on the same route. Domain-root sites unchanged.
- [#362](../../issues/362) — A `srcset` candidate whose URL carries a comma (a `data:` URI, a `?w=100,200` query) survives the cachebreak and base-path passes whole. Both parse candidates through ONE shared splitter that reads a comma inside a URL as part of the URL.
- [#364](../../issues/364) — A site served under a URL path notices its own new deploys: the refresh-new-version poll fetches `<prefix>/build.json` instead of a root-relative path that 404s forever. The prefix is read from the same `<html>` stamp the service-worker registration reads, through one shared helper. Domain-root polling unchanged.
- [#368](../../issues/368) — The runtime cache-buster stops rewriting URLs it does not own: `data:`, `blob:` and external URLs pass through a lazy load untouched, so a `data:` URI payload is no longer corrupted by an appended `?cb=`. Same locality rule the build-time cachebreak pass has always used.
- [#367](../../issues/367) — A lazy-loaded `srcset` candidate whose URL carries a comma (a `data:` URI, a `?w=100,200` query) survives the RUNTIME rewrite too: the lazy-loader reads candidates with the srcset grammar instead of splitting on every comma. The browser grammar is the ESM twin of the build passes' splitter.
- [#366](../../issues/366) — `omega deploy` warns on a project site with no `brand.url`, printing the exact Pages URL to set so canonical, `og:url` and sitemap URLs resolve. Setting it is safe: a `*.github.io` host is never a custom domain, so the project shape holds.
- [#360](../../issues/360) — A site served under a URL path keeps its service worker: the client registers `<prefix>/service-worker.js` at scope `<prefix>/` and passes the prefix on that URL, so the worker mounts what it builds — `/build.js`, the notification icon, the click target. Domain-root registration unchanged.
