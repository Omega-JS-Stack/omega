...
- [#577](../../issues/577) — Every match parameter Meta, TikTok, and GA4 accept is plumbed, one normalization table per provider; the fire log names which match keys went (key NAMES only); the dev palette gains an **Ad match keys** readout.
...
### Changed
- [#637](../../issues/637) — A trial-eligibility check that times out or fails is UNKNOWN, not "not eligible": checkout quotes no trial and charges the full amount today, while the intent payload still asks for the trial so the server grants it to a buyer who qualifies.
- [#639](../../issues/639) — ONE key per AI provider: `OPENAI_API_KEY` and `ANTHROPIC_API_KEY`, the `OMEGA_*` twins removed outright, a company-wide key served by the company `.env` layer, and a new `ai` manage service that asks for both once (`ai.enabled: false` to stop asking).
...
- [#577](../../issues/577) — The server half of `sign_up` fires from the post-auth request (`routes/user/signup`), not the auth trigger, so Meta and TikTok get IP, user agent, and platform cookies (`attribution.cookies`); same `sign_up.<uid>` dedupe id, one fire per account.
...
### Fixed
- [#670](../../issues/670) — `omega dev` on a linked brand waits for the monorepo watch's initial prepare before booting any target, so a leg no longer loads a `dist/` the watch is still rewriting. Bounded at 120s, with one warning naming the packages still preparing.
...
- [#635](../../issues/635) — Every value a brand must supply is ASKED for at the moment of need instead of warned about and skipped: the TikTok app id and redirect URI, an ambiguous Sentry organization, the Apple bundle id prefix, and the CAN-SPAM mailing address. `OMEGA_WEBHOOK_KEY` is minted in place, never asked for.
...
- [#638](../../issues/638) — Signup fills `personal.location.country/region/city` from the request's geolocation when they are empty, so the ad-platform match keys (`ct`/`st`/`country`) and the email merge fields stop shipping blank — a location the user set is never overwritten.
- [#633](../../issues/633) — The post-auth signup request fires once per account: a per-uid in-flight marker (`temporary.signupMetadata.<uid>`) gates the post while the server writes `flags.signupProcessed`. A failed send clears the marker on the spot, and a marker older than 10 minutes counts as absent.
- [#632](../../issues/632) — A log line names a user by address or uid and a payload by its key names, never by serializing the document: `Email.send()`, the signup lines, the `Usage` lines and `test/authenticate` used to write `api.privateKey`, consent, the request IP and attribution into Cloud Logging in plain text.
