...
### Added
- [#415](../../issues/415) — Cancellations and refunds reach Meta and TikTok as zero-value custom events (`SubscriptionCancelled`, `Refunded`) so an ad account can build exclusion audiences from them, and the analytics doc gains a "Running paid ads" playbook covering the utm rule, the two ledgers and the shipped Measurement Protocol secret.
- [#407](../../issues/407) — Four subscription lifecycle moments now reach analytics: a plan change carrying the plan it came from, a withdrawn cancellation as the retention win it is, a trial suspended by a failed charge, and the first real charge after a trial as its own conversion instead of a routine renewal.
- [#405](../../issues/405) — A signed-in user whose Firestore doc went missing gets it back at the next request: the backend recreates the doc a signup writes from the Auth record, then authenticates them normally. Signups in flight, anonymous accounts and uids with no auth user are left alone.
- [#402](../../issues/402) — The dev palette waits for a backend that is still booting: while the persona roster is unreachable, a spinner and a "Backend starting…" line sit at the top of the panel and the fetch retries every two seconds, filling the dropdown and clearing the indicator the moment the emulator answers.
- [#401](../../issues/401) — The account's referrals card shows the inbound half too: a "Referred by" line naming the code the account itself signed up through and when it was used, hidden for an account nobody referred.
- [#380](../../issues/380) — The `@omega.js/monitoring` internal package: one error-reporting contract for every target. Config resolution from `monitoring.*` (an unset DSN means the SDK is never loaded at all), one release tag builder, the PII scrub, and per-platform entries for the Node process, the Electron main/renderer/preload contexts, and the browser bundle.

### Changed
- [#380](../../issues/380) — Client-side errors report only when our own framework code is on the stack: a stack-frame filter against the `@omega.js` bundle URLs drops user-land scripts, third-party widgets, visitor browser extensions, and cross-origin `Script error.` noise. Server and framework errors still always report.
- [#380](../../issues/380) — Emails are scrubbed from error reports by default on every surface; the uid still rides. Set `monitoring.scrubEmail: false` to opt back in. The client used to attach the signed-in email unconditionally.
- [#380](../../issues/380) — `@omega.js/web` hands the client its error-reporting config: the page `Configuration` block maps `monitoring.*` to the client's `sentry` contract, the mapping `@omega.js/extension` already had. A brand's `targets.web.monitoring.dsn` was previously read by nothing.
- [#380](../../issues/380) — `@omega.js/desktop`'s `src/lib/sentry/` is gone; the framework requires the shared package directly. Its `./lib/sentry` export is retired.
- [#380](../../issues/380) — Every target tags Sentry releases `<brand.id>@<version>`: a desktop app's tag gains its brand id, an extension's carries its app version instead of the build stamp. Web pages and the desktop renderer bake the app version too, so no surface tags a build stamp.
- [#398](../../issues/398) — A CLI boot heals a stale monorepo-linked dist when the monorepo watch is down: one prepare under the heal lock, then the usual re-exec, instead of the loud stop. With the watch running that stop is unchanged, and the watch-down warning still fires.
- [#398](../../issues/398) — A monorepo-linked heal that FAILS stops the boot: exit 1 naming the package, the broken prepare and the `npm run prepare` to run by hand. `omega dev` spawns no leg on it; plain checkouts still warn and continue.
- [#398](../../issues/398) — The watch-down warning no longer claims a heal that did not happen: a stale linked dist is healed once at boot, and nothing rebuilds it as you edit.
- [#400](../../issues/400) — The dev palette's account switcher fetches its persona list from the seed at `GET /omega/test/roster` instead of hardcoding one: the backend labels every human-facing persona, the Referrer and Referred pair included. With no emulator up the dropdown keeps its placeholder and the panel says why.
- [#394](../../issues/394) — A data migration writes only when you say so: `omega manage --migration` is the audit pass — every fix computed, counted, logged and snapshotted, and not one document touched — and `--execute` is the only path that writes. The old write-by-default, previewed with `--dry-run`, is gone.
- [#396](../../issues/396) — Every surface's anonymous device id comes from one derivation in `@omega.js/analytics`, with each target injecting its own storage and seed: desktop keeps its MAC seed, and a browser now persists a real UUID instead of falling back to a random-looking string on an insecure origin.

### Fixed
- [#409](../../issues/409) — Meta and TikTok page views go through the analytics facade instead of the raw pixel calls the loader fired at init, so the catalog and the per-event consent gate cover them, and GA4 still counts one page view from its own config command.
- [#411](../../issues/411) — A desktop window's own `omega.analytics()` calls now count: they forward over IPC to the main process, whose sender fires them with the install's one device id and one session id, where before they reached nothing at all. A renderer never talks to GA directly and never holds the api_secret.
- [#414](../../issues/414) — A cancellation inside the trial term books `trial_lapsed` on every processor instead of counting a customer who never paid as churn, the cancellation that ends a lapsed trial's dunning books nothing rather than a second story, and a renewal whose payload arrived without a term books its revenue.
- [#408](../../issues/408) — The signup funnel entry and the 404 miss, both shipped in 0.42.0, are pinned to their call sites against regression: one entry on the first interaction, one miss carrying the path asked for, and no shared module counting either.
- [#399](../../issues/399) — A payment never creates a user doc: the webhook pipeline refuses a uid with no Firebase auth user, stamps the event and reports it, and a checkout missing an auth user or user doc is refused, so QA checkouts stop leaving orphan accounts in live projects.
- [#380](../../issues/380) — Event-trigger failures reach Sentry: a handler that throws, or a handler file that will not load, now goes through the same `report()` door a 5xx route does instead of only logging. A deliberate `HttpsError` block stays the trigger's 4xx and never captures.
- [#388](../../issues/388) — GA4's Measurement Protocol payload carries `sha256_phone_number` again: the helper read a `personal.telephone.number` no account has ever had. It now reads `national` and hashes the E.164 number, while an account with no phone still sends no match key. The dead `options.userData` parameter is gone.
- [#397](../../issues/397) — GA4's `sha256_email_address` is the digest of the normalized address: an email stored with a capital or a stray space used to hash raw and match nobody.
- [#403](../../issues/403) — GA4's `sha256_first_name` and `sha256_last_name` are the digests of the trimmed, lowercased names its user-data spec asks for, so a stored capital stops silently missing the match.
- [#394](../../issues/394) — Documents written before the touch model stop reading as no campaign at all: `omega manage --migration` folds a legacy `attribution.utm` blob into `first`/`last` touches on users, notifications, `payments-orders` and `payments-intents`, then deletes the blob. No empty husks are invented.
- [#410](../../issues/410) — TikTok gets `external_id` SHA-256 hashed, the shape its Events API requires and a raw id silently failed to match: the server conversion and the browser pixel hash the same uid the same way, so the two halves still link. Meta's stays raw, which is what its own spec asks for.
- [#392](../../issues/392) — Server conversions hash a phone per platform spec instead of sending one digits-only digest to everyone: Meta keeps the bare digits, TikTok gets E.164 with the plus. Both use the `@omega.js/analytics/identity` normalizers the browser pixels already use, so both halves present identical match keys.
- [#365](../../issues/365) — Newsletter readers stop being deleted on offer-mail silence: the monthly prune is strictly per provider now, so the SendGrid lane deletes only from SendGrid and a new Beehiiv lane deletes only the subscribers Beehiiv's own per-subscriber stats call cold, each behind its own received-count floor.
- [#393](../../issues/393) — A standalone devkit vendor-hook run stops wiping an already-prepared `dist/vendor/`: a pass that finds nothing new to vendor leaves the tree intact, and a watch-mode single-file copy re-vendors its own seed by copying over the top instead of resetting the directory.
- [#395](../../issues/395) — `omega dev` legs keep their colors: the fan-out spawn now passes `FORCE_COLOR` through when the parent owns a real terminal, so the backend and web log bodies stop printing plain under piped stdio while only the leg prefix stayed colored.
- [#389](../../issues/389) — The trial-lapse-sweep suite's converted-trial case waits for the swept doc the way its sibling cases do, closing the full-lane flake where the read raced the write.
