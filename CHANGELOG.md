...
- [#645](../../issues/645) — One doc per manager service in `docs/manager/`: what each of the 28 reconciles, its config keys, its gotchas, with the map linking them. The edge doc carries the subdomain gotcha: a two-label api host stays DNS-only, so Cloudflare geo headers never arrive. A test holds the parity.
- [#626](../../issues/626) — `requiredWhen` on env schema entries: a key must be non-empty when its config path is set. One checker feeds the backend boot (production refuses), the desktop and extension bakes (build mode refuses), and the manage walk (warns per enabled target). Replaces the scattered hand checks.
...
- [#448](../../issues/448) — TikTok setup mints its own token: paste the app secret once (never saved), walk the portal authorization, exchange inline, and only `TIKTOK_ACCESS_TOKEN` reaches the brand `.env`.
...
- [#644](../../issues/644) — The backend wakeup ping fires from every entry point whose first user action is a backend call: eight web pages, the newsletter band, download, the desktop auth bridge and the extension surfaces. One shared `WAKEUP_ROUTE` (`/omega/health`) from `@omega.js/client` replaces web's `PAYMENT_WARMUP_ROUTE`.
- [#662](../../issues/662) — A manage service WAITS for what it needs instead of owing a rerun: SendGrid's branded-link validation before proxying `emailurl`, a new zone's nameservers before the registrar write, GA's release of a just-deleted Firebase link. Each wait skips with `s`, warning with its reason.
- [#627](../../issues/627) — How an env key reaches each target is one schema declaration (`delivery`: env, ci, or bake): one renderer derives every workflow secrets block, bake list, and published secret set. A secret only bakes when declared public at rest; desktop's workflow now composes to the brand root.
- [#678](../../issues/678) — The brand-root `.env` is the one file humans and the manager edit; a target `.env` is an optional per-key override no machine writes. Every verb composes the target's runtime env from the schema-filtered file cascade, `deliverAs` renaming on delivery; the CI secret publishers read the same composed env.
- [#675](../../issues/675) — `omega setup` is retired on every framework: its local half runs as `ensureTarget()` at the start of every verb, its network half as a deploy precheck with `--no-secrets` to skip it. A bare `omega` prints help; backend's setup checks ride `omega test`; extension migration is its own `omega migrate`.
...
### Fixed
- [#691](../../issues/691) — The mcp-router oneshot deadline test stops flaking under lane load: its elapsed-time assertion gets 10ms of slack for Node firing timers a hair before Date.now agrees.
- [#690](../../issues/690) — `omega dev`'s programmatic shutdown no longer leaks the firebase emulator: each leg spawns in its own process group and shutdown signals the group, so the grandchild dies with npm instead of orphaning onto the classic ports.
- [#688](../../issues/688) — The dev-watch suite stops flaking on a lost first edit: a save landing before the fresh watch root's FSEvents stream initializes is re-fired until the watcher wakes, and a woken-but-stale rebuild still fails.
- [#675](../../issues/675) — A freshly onboarded brand boots: the workspace walk fills each target's missing npm scripts from its framework's `projectScripts` declaration, so `omega dev`'s fan-out finds its legs before any per-target verb has run.
...
- [#666](../../issues/666) — A subscription that sells no trial stops asking `/omega/payments/trial-eligibility` on checkout: the question only goes out when the product's `trial.days` is above 0, so a trial-less plan prices itself out of build config with no authed round trip and no skeleton wait, exactly like a one-time buy.
...
- [#643](../../issues/643) — The manage run summary names what warned: each service's breakdown lists its warned operations with the reason each gave, instead of one "some operations had issues" line per service.
- [#680](../../issues/680) — The extension deploy precheck publishes its store credentials and GA secret as repo Actions secrets, like web and desktop, so a first `publish.yml` run stops failing on secrets nobody set. `--no-secrets` opts out.
- [#679](../../issues/679) — The live-page feed poller pauses in a covered tab and resumes with one immediate read on return, so a background dashboard stops burning its host's rate limit. A visible-but-unfocused window keeps polling.
- [#668](../../issues/668) — One-time purchases stop being priced and labelled as subscriptions: `once` from the pricing click through checkout, the intent and the receipt; `begin_checkout` carries the real price instead of $0; the receipt names the product; and the confirmation page opens confirmed instead of polling for a plan.
- [#665](../../issues/665) — A detected transition is claimed on `payments-orders/{orderId}.transitions.<name>` in a transaction: the loser dispatches no handler, fires no analytics and runs no marketing sync, so a concurrent checkout pair sends one order email, not two. The claim speaks for ten minutes, and a failed run's retry retakes its own.
