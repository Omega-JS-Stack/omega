...
- [#584](../../issues/584) — `targets.backend.projectType: 'custom'` runs the backend as its own server on `PORT` for a container host (Render & co): same routes, same auth middleware, no Cloud Functions. Deploy and test go through the target's own scripts; the Firebase-only verbs refuse and name their lane.
- [#612](../../issues/612) — `omega migrate` at a brand root deletes every retired key from `config/omega.json5` in place: the key, its subtree and the comment documenting it, one line each naming the replacement. Idempotent, `--dry-run` for the plan, every other byte preserved.
- [#522](../../issues/522) — `omega migrate:rules` (alias `migrate:firestore-rules`): the run-alone backend verb that moves a legacy `firestore.rules` onto the compiled model and repoints `firebase.json`, loud about the live-posture change it makes.
- [#587](../../issues/587) — A brand-root `omega dev` starts the monorepo watch as a session child whenever the brand's `@omega.js/*` deps resolve into a monorepo checkout: one terminal now covers framework edits too. Lock-aware, dies with the stack; a registry-installed brand says so in one line.
- [#529](../../issues/529) — The translate engine dedupes a page's cold strings before batching: each unique string is translated once and fanned back to every occurrence, so a title sent by three meta tags stops costing three translations. The #523 split-and-re-ask stays the backstop.
- [#567](../../issues/567) — `omega setup` scaffolds the backend's socket-free STATIC test lane: the connect trap (any TCP connect or DNS lookup throws), three skeleton suites under `test/_unit/`, and a `test` script that preloads the trap. `npm test` is that lane now; the emulator lane is `npm run test:emulator`.
- [#441](../../issues/441) — The hero takes a consumer-supplied animated visual: one folder at `src/_hero/<name>/` (`index.html`, `style.scss`, `script.js`, bundled like section assets), named by `demo: { type: custom, name }`. The framework ships `orbit` as the reference, reduced-motion branch included.
- [#513](../../issues/513) — The marketing/hero section takes an optional `breadcrumb` trail: an ordered `{ label, href }` list rendered as a labeled nav above the headline, the last crumb the current page and unlinked; absent renders nothing.
- [#515](../../issues/515) — New `marketing/prose` section: the standard head cluster over a `body` of paragraphs, for the head-plus-lede band that previously had to fake itself with an item-less showcase.
- [#493](../../issues/493) — New `marketing/pricing-cards` section, composed by the base index layout under the WHY band: one compact card per plan, read from the same resolved catalog `/pricing` renders, so no homepage number is ever a copy.
- [#514](../../issues/514) — A marketing/bento tile takes an optional `href` and becomes ONE anchor wearing the shared interactive states, so a hub or category listing composes the band instead of hand-rolling a grid of linked cards; a linked tile's body carries no nested anchors.
- [#519](../../issues/519) — The marketing/bento band takes `numbered: true`, auto-numbering its tiles 1..N by position as a small ordinal chip, so a process band keeps its steps and reordering renumbers itself.
- [#518](../../issues/518) — marketing/stats items take an optional `icon` and a `color` naming a slot in the categorical token palette (never a raw hex), so a stat rail keeps the glyph and accent its legacy carried.
- [#550](../../issues/550) — A REFUSED payment webhook reports itself to Sentry as ONE warning carrying the refusal reason, the provider and the event ids, so a forged owner or a resource the provider never had surfaces on the dashboard, not just in the logs. A processed event, or an unset DSN, reports nothing.
...
- [#581](../../issues/581) — `@omega.js/config` gains the env schema: one inventory of every env key OMEGA needs — owner, targets, minted or third-party, secret, required, description. The manager's mint, `.env` grouping and disperse composition, plus the backend's reader, all derive from it: a new key is one entry.
...
- [#462](../../issues/462) — `socials` is empty by default and the footer loops only the entries a brand defines, replacing the hardcoded five-platform fallback that drew icons for accounts a brand never had.
- [#463](../../issues/463) — The section library at `/test/sections`: one page per section, every variation in its own autosizing frame with its authored args beside it, referenced from the sections docs.
- [#470](../../issues/470) — `omega migrate` names the retired `asset_path` frontmatter key and its replacement instead of reporting it as an ignored content key indistinguishable from copy.
- [#472](../../issues/472) — The about layout gains a standalone stats band with its own head cluster, so a legacy "By the numbers" heading has a home instead of vanishing.
- [#475](../../issues/475) — `hero.demo.placement: side` renders the demo beside the copy; the default stays centred under the CTAs.
- [#476](../../issues/476) — Hero buttons render their authored `icon` and `class` again (the legacy `rocket` + `btn-light` shapes); unauthored buttons stay byte-identical.
- [#478](../../issues/478) — Self-healing config: every manage run materializes schema-defined defaults into `omega.json5`, so a consumer's config stays complete as the schema grows.
- [#479](../../issues/479) — One machine-record file, `.omega/state.json`, sectioned per fact kind: deploy records move back in from the `.omega/deploys.json` that 0.45.0 introduced.
- [#480](../../issues/480) — A bridged desktop renderer's `setUserId` throws, naming main's `manager.analytics.setUserId(uid)` as the call to make, instead of silently no-oping.
- [#486](../../issues/486) — `hero.demo.type: video` renders a video demo in the side or centred column.
- [#496](../../issues/496) — A side-placed hero demo and the centred visual are either/or: side wins and the visual moves below the fold.
...
- [#606](../../issues/606) — The page chrome no longer defines empty `gtag`/`fbq`/`ttq` for an unconfigured provider. The guarded browser transport already reads a missing global as a silent skip, so the stubs only made a pixel that never loaded look present.
- [#610](../../issues/610) — `targets.web.download` and `targets.web.extension` are retired, and `site.download`/`site.extension` are gone. The `/download` page, the `/extension` page and their shortlinks read the curated `site.targets` view alone. A config still carrying either key fails validation naming the block it derives from.
...
- [#604](../../issues/604) — Translation batches run `CONCURRENCY` wide (5) instead of one at a time, results stitched back in batch order. The sentinel, alignment checks and the #523 split backstop stay per batch, unchanged.
- [#605](../../issues/605) — The framework excludes its OWN default pages from translation — auth, the app shell, account, payment, portal, legal, redirect stubs — derived from the defaults tree. `translation.exclude` is for brand pages only now.
...
- [#581](../../issues/581) — The backend reads every brand-supplied env key through one reader (`libraries/env.js`), and `Manager.init()` refuses to boot in EVERY environment — development included — naming every missing required key. **A brand whose `.env` lacks a minted key must run `npx omega manage` before its backend boots.**
...
- [#527](../../issues/527) — AdSense has ONE switch: `advertising.providers.adsense.client` presence drives the managed account, the rendered units and the `ads.txt` record together. `advertising.providers.adsense.enabled` is deleted — it no longer skips the manager's service, and the interactive Disable answer opts the provider out (`advertising.providers.adsense: false`) instead. The gating doctrine behind it is in `docs/shared/config.md`.
- [#539](../../issues/539) — The homepage plan band renders `/pricing`'s feature lists through the same new `pricing/features` component, so a feature the catalog explains carries its hover definition on the homepage too. The line introducing a tier's extras reads "Everything in <previous>, and:" on both surfaces.
- [#540](../../issues/540) — The section gallery is a rail plus a content pane that stacks EVERY variant, one frame each with its options, over the args table. The rail's variant links are plain anchor jumps, the gallery head clears the site masthead, and the "← Section library" back link is reachable again.
...
- [#549](../../issues/549) — `/test/components` is the component gallery: every resolved `_components` entry on one page, wearing the section gallery's rail-plus-stacked-frames shell. The living styleguide that held that URL moved to `/test/styleguide`, whole, with the same dev-page rules.
- [#490](../../issues/490) — A website target's link exceptions live in `config/link-exceptions.json5`: json5 like every other consumer surface, so the reason an exception exists travels beside it in a comment instead of some worksheet. A file left at the retired `.json` name fails loudly with the rename.
- [#538](../../issues/538) — Every `omega_icon` wrapper ships `aria-hidden="true"`, so decorative icons leave the accessibility tree by declaration instead of by the convention that screen readers skip text-free SVGs; the rare meaningful icon passes `label="…"` and renders `role="img"` + `aria-label` instead. One mechanism change, no call-site edits.
- [#555](../../issues/555) — A gallery frame carries no overlay chrome: the frame page bakes the consent gate and the chat widget off beside the nav and footer it already drops, and `main.js` skips the dev palette inside an iframe. A normal page still ships all three.
- [#554](../../issues/554) — A production build emits NO page at `/test` or under `/test/` (framework dev surfaces and consumer overrides alike), the omission the `/test/sections` gallery already rode. Dev builds keep them all; the boundary is a path segment, so `/testimonials` ships.
- [#547](../../issues/547) — Markdown renders with `typographer: true` on both markdown-it instances, so a brand converted off Jekyll keeps Kramdown's smart quotes, apostrophes, dashes and ellipses instead of drifting into straight punctuation on its next build. No consumer knob. **This changes rendered punctuation on every brand at its next build.**
- [#573](../../issues/573) — A scaffolded extension's `config/messages.json` seeds `appDescription` from `brand.description` when it fits the 200-character store cap, so the one field every consumer hand-edited arrives written. A longer or absent description keeps the generic phrasing.
- [#576](../../issues/576) — A compiled extension manifest carries `homepage_url`, baked from `brand.url` on every browser target, so the Chrome and Firefox store listings link the brand's site. A declared value wins; no `brand.url` ships no key.
- [#583](../../issues/583) — An extension's default `externally_connectable.matches` is the brand's own origin from `brand.url`, with the localhost dev origin in dev builds only, so a brand site can message its published extension. A declared consumer value still wins; a scaffolded boot test asserts the brand origin.
- [#481](../../issues/481) — docs/shared/breaking-changes.md names the backend test-discovery change: only `*.test.js` files run, so a plain `.js` legacy test needs renaming or it is silently invisible.
- [#483](../../issues/483) — docs/shared/config.md maps legacy `socials` to the root key the schema defines, not `targets.web`.
- [#484](../../issues/484) — The `testing` config key, read by nothing, leaves the canonical order; the legacy `testing.apiCheck` and `testing.endpoints` keys are named as retired.

### Fixed
- [#615](../../issues/615) — The web watch suites derive their rebuild deadline from the machine's measured contention (30s floor × load per CPU, capped) instead of a flat 30s, and every timeout names the elapsed time, the derivation and the last rebuild event seen.
- [#614](../../issues/614) — `omega setup` on a `projectType: 'custom'` backend scaffolds no Firebase-only artifact — no `firebase.json`, rules, or index files — and drops the checks that maintain them. An authored one is kept, untouched.
...
- [#613](../../issues/613) — `site.time` is set once per build, so a post's `article:modified_time` and its JSON-LD `dateModified` carry a real date instead of the empty string every brand shipped. Both name the same instant as `site.omega.date.iso`.
- [#611](../../issues/611) — `omega migrate`'s `config-reads` rule rewrites every `site.<section>` read (page bodies, frontmatter, section descriptors) to `resolved.config.<section>`, and the build now fails on a missed one naming file, line and expression. Only Liquid context counts: a hostname, a filename, a fenced code sample and a `{% raw %}` body are not reads.
- [#579](../../issues/579) — `omega migrate` writes `enabled: true` onto a `hero.secondary_button` block that has no `enabled` key, and the report warns that it did. `marketing/hero` defaults the button off and a UJM page opted in by authoring the block, so migrated pages built clean with the CTA gone. The section default is unchanged.
- [#565](../../issues/565) — `omega migrate` converts a class-only Kramdown attribute list on a link — `[text](url){: .a .b }` → `<a href="url" class="a b">text</a>` — in markdown files only, and reports every other surviving `{: … }` as an error. The engine still loads no attrs plugin.
- [#559](../../issues/559) — `omega migrate` converts the retired adunit includes: `{% include modules/adunits/adsense.html type=… vert-size=… %}` becomes `{% section "verts/unit", type: …, size: … %}`. An arg the section has no home for is a warning, and any other `adunits/*` include an error — an unresolved include renders nothing and fails nothing.
- [#560](../../issues/560) — `omega migrate` rewrites the UJM library accessor: `webManager.uj().X()` / `omega.uj().X()` become `omega.library().X()`. A `.uj(` the rule cannot claim — an unknown receiver, a call with args — is an error finding naming the successor, not a build-clean file whose first call throws.
...
- [#548](../../issues/548) — A web build caches the six Firebase auth helper files per project under the brand's `.omega`, and a failed fetch now serves that copy with a dated warn instead of failing a build that already emitted every page. A failure with nothing cached stays fatal; `demo-*` projects still skip.
- [#546](../../issues/546) — The three service switches the manager reads are declared in the schema at last: `repo.providers.github.enabled`, `search.providers.searchConsole.enabled` and `edge.providers.cloudflare.enabled`, boolean, default on. A misspelled one now fails validation instead of silently reading as on.
- [#509](../../issues/509) — A payment webhook's write is steered by the uid on the PROVIDER's record; a payload claiming a different one refuses the event outright, stamped `uid-mismatch` and loud, so an event naming a real subscription with a forged uid stops moving it onto that uid.
- [#510](../../issues/510) — A refund's amount, currency and reason come from provider data — Stripe's charge, PayPal's own refund record, Chargebee's credit note — so a payload claiming an inflated refund stops booking that number onto the order, the refund email and the conversion.
- [#511](../../issues/511) — The backend mcp and verts suites address the lane they booted (`config.apiUrl`) instead of a hardcoded `localhost:5002`, so a port-bumped run stops firing its 24 tests at whatever holds the classic port.
- [#495](../../issues/495) — The backend cron-job recipe documents the shape the runner actually calls (an async function taking `{ Manager, ctx, context, libraries }`), so a job written to the doc stops silently never running; consumer paths in `routes.md`/`file-naming.md`/`test-framework.md` are src-first, and the consumer tree names `payment-providers/`.
- [#522](../../issues/522) — `omega setup` no longer migrates a legacy `firestore.rules` in place: while `firebase.json` still points at the brand's own file, both rules checks report the deferral with the migration pointer and leave every byte alone, so a deploy chain can never flip live rules posture unasked.
- [#500](../../issues/500) — The marketing/hero headline no longer hardcodes `h1`: a `heading_level` arg (default 1) lets a hero-shaped band below the fold author 2, so a page composing the section twice ships one h1 instead of two.
- [#491](../../issues/491) — The `frontend/core/minimal` layout gates its masthead h1 on the breadcrumb being present, so a document page with none stops shipping a blank first h1 above its body.
- [#512](../../issues/512) — A marketing/bento band with no `subheadline` renders no sub line at all instead of inheriting the section's demo sentence; the demo copy moved to the gallery variant, and the same sweep emptied the about timeline and principles sub lines.
- [#520](../../issues/520) — The about/letter aside gates on having something to say: empty `feed_items` removes the feed frame, status chip and label, and with no aside photo either the aside column goes too and the letter runs full width — no more empty ornament on a brand that only wanted the mission copy.
- [#516](../../issues/516) — The about hero's facts head passes `headline_accent` through like every other head composition, so an accented stats headline stops rendering without its accent word.
- [#517](../../issues/517) — The team hub gives its brand copy somewhere to land: an optional head cluster over the portrait grid, an optional one-line `bio` per member under name and position, and an optional headline over the mission band; all absent renders the hub exactly as before.
- [#499](../../issues/499) — Autoplaying video parks under `prefers-reduced-motion`: the shared motion engine strips autoplay, pauses the player and shows its controls for every `video[autoplay]` on the page, so the hero video demo and marketing/product-demo stop moving for a visitor who asked for stillness.
- [#521](../../issues/521) — The internal link check reads `<pre>` blocks as code DISPLAY, not markup: an escaped `href=` inside one (a docs snippet, the section gallery's copyable args) is characters on the page, so a demo variant can author a real link again — a real anchor at the same URL still fails.
- [#488](../../issues/488) — One slugifier answers a taxonomy term on both sides: the term is LINKED at the slug its page is GENERATED at, so `A&R` stops shipping `/blog/tags/a-and-r` against a page written at `/blog/tags/a-r`. Eleventy's own `slugify` no longer shadows the framework's in templates.
- [#489](../../issues/489) — `omega migrate --check` knows the tags the lane converts INTO: `{% section %}`, `{% component %}` and `{% composition %}` are registered in the lint pass, so a fully converted tree stops reporting its own output as unknown (71 warnings on a clean port).
- [#523](../../issues/523) — A translation batch the model keeps collapsing is re-asked in halves instead of failing the page: a merged pair returns the same short array on every retry, which shipped `/blog/ship-the-docs-with-the-diff` untranslated in both languages. A single string that still will not come back whole fails loudly.
- [#504](../../issues/504) — The root layout stops stamping `data-theme-target`, a dead attribute interpolated from a legacy key the config schema never had — nothing reads it, and a correct brand file rendered it empty.
- [#485](../../issues/485) — A DSN at `monitoring.providers.sentry` now wins the page `Configuration` blob: the web foot emits it AFTER the `resolved.client` loop, so a stale `client.sentry` stops silently overwriting the DSN the monitoring service provisioned. A brand with no canonical DSN keeps reporting from its legacy client blob exactly as before.
- [#531](../../issues/531) — The neobrutalism and newsflash `.omega-price-card` skins moved out of the /pricing page sheet into the theme's marketing layer, so the homepage plan band wears the theme's cards instead of a plain themed `card`; /pricing renders identically from the shared layer.
- [#530](../../issues/530) — The #512 sweep finishes: `marketing/showcase`, `marketing/product-demo` and `marketing/pricing-cards` ship an empty `subheadline` default, so a band composed without one renders no sub line instead of the section's demo sentence; each demo line moved to that section's gallery variant.
- [#524](../../issues/524) — The config schema declares the keys real brand files already carry — `brand.type`, `brand.font`, `brand.contact.phone`, `cloud.oauthRedirectsConfigured`, the google/meta/tiktok platform account ids and the chatsy owner account — so a typo in one fails loudly instead of validating by not being looked at, and each key documents what reads it.
- [#528](../../issues/528) — The backend event-handler recipe documents the real `src/manager/events/` tree and the shape EventMiddleware actually calls (an async function taking `{ Manager, ctx, libraries, user, context, change, snapshot }`), so a handler written to the doc stops silently never running; the consumer lane and its src-first authored path are named too.
- [#532](../../issues/532) — Every refund lookup is linked back to the resource the event named — PayPal's `sale_id`/`links[rel=up]`, Chargebee's `subscription_id`/`reference_invoice_id`, Stripe's charge `subscription`/`metadata.uid` — and a record belonging to another order is refused, stamped `refund-not-linked`, so an unrelated refund id stops importing another customer's amount onto this order.
- [#533](../../issues/533) — A Chargebee payload uid is checked against the hosted page's `pass_thru_content` BEFORE it is allowed to steer: a page that names another owner refuses the event, and a page outside the 25-entry scan window still falls back to the payload with the loud warn.
- [#534](../../issues/534) — A Chargebee refund with no credit note reads its amount and currency from a trusted `GET /transactions/{id}` (the payload's transaction id as a lookup key only), so a genuine gateway refund stops recording `amount: null` on the order and in the customer's refund email.
- [#535](../../issues/535) — A REFUSED webhook whose refusal stamp fails to write no longer falls back to marking a payload-named `payments-intents` doc failed: the refusal is terminal, nothing else is written, and the retry sweep re-lands the stamp.
- [#536](../../issues/536) — A Stripe refund envelope carrying no charge id fails as a PERMANENT failure before any `charges.retrieve` call — dead-lettered on the first attempt with the envelope's actual shape in the log — instead of classifying as unreachable and burning the whole retry ladder.
- [#541](../../issues/541) — The `page.<key>` codemod tracks Liquid block scope: inside a `{% for %}`/`{% tablerow %}` binding the same name the rename is skipped and reported with file, line and hand-port, so `{% if post.id != page.post.id %}` stops becoming a self-comparison. Occurrences outside the loop still rewrite.
- [#545](../../issues/545) — `omega migrate` maps the three renamed section args — `hero.tagline` → `hero.badge.text`, `cta.description` → `cta.subheadline`, `cta.button` → `cta.primary_button` — so a migrated page keeps its eyebrow, CTA sub line and CTA button. A hero already carrying the legacy `badge:` object is reported for a by-hand merge.
- [#551](../../issues/551) — Two default-true switches read backwards now read `!== false`: the web consent gate (`client.consent.enabled`) and the monthly marketing prune (`marketing.prune.enabled`) stay ON for a config that never mentions the key, so a config built outside the loader stops silently dropping the banner and skipping the prune.
- [#556](../../issues/556) — The generated `/ads.txt` opens on its record: the gate tag right-strips too, so every brand's file stops starting with a blank line. The no-provider comment gets the same treatment.
- [#553](../../issues/553) — `devlog.enabled` is declared in the config schema — boolean, optional, no materialized default — so a misspelled switch fails validation instead of validating clean. Absence still means off; only a literal `true` publishes devlog posts.
- [#542](../../issues/542) — Frontmatter and `.11tydata.*` sidecar Liquid renders with the consumer's own `_data` globals in scope — the same globals a template render gets, `site` included — so `{{ brands.size }}` beside a `_data/brands.json` stops rendering empty with no warning.
- [#543](../../issues/543) — The sidecar array-replace repair covers every extension: a `.11tydata.js`/`.cjs` sidecar array REPLACES the layout default exactly as the `.json` twin does, instead of concatenating a 6-item list onto the default into 7.
- [#544](../../issues/544) — A `meta:` ref rooted at the page's pagination alias defers to render time; any other meta ref with an undefined root fails the build naming file and ref, never caching an empty render. Empty optional values still pass.
- [#557](../../issues/557) — A rejected reCAPTCHA verify prints Google's verdict as ONE warn line — `error-codes`, `score`, `action`, `hostname`, never the token or secret — so a captcha 403 names its own reason. The boolean return is unchanged; a pass stays silent.
- [#558](../../issues/558) — The checkout order summary prices a one-time product by its one-time price (`$49.99 one-time`) instead of the URL's default cadence, which read `$0.00 annually` over a $49.99 SUBTOTAL. Subscriptions keep their `$X annually|monthly` line.
- [#526](../../issues/526) — `omega migrate` converts `translation` to the shared ROOT, not `targets.web`, finishing the #483 shape: disperse copies the SHARED sections, so a target-scoped engine config never reached the other targets that translate. Doc row and converter now say one thing.
- [#566](../../issues/566) — The response-headers doc names `ctx.report()`, the method that exists: the header line stopped pointing a porting consumer at BEM's removed `errorify()`, which throws `TypeError` on the first error it handles.
...
- [#574](../../issues/574) — The extension docs and the scaffolded manifest comment describe what packaging does today: the firefox `gecko.id` is DERIVED from the brand when none is declared, failing only with no brand facts at all. The "declare it before the first publish" caveat stays, with its reason.
- [#572](../../issues/572) — `omega setup` writes an extension consumer's `package.json` with npm's own trailing newline, and only when the content changed. Every build re-stripped that newline, so a consumer's editor or lint hook fought setup on every run.
- [#575](../../issues/575) — The extension boot layer skips as its docs promise when nothing is built: a directory qualifies only when its `manifest.json` is strict JSON, so the JSON5 `dist/` copy left by any dev run stops being loaded and failing every boot test. An explicitly named `OMEGA_TEST_BOOT_DIR` still fails loudly.
- [#582](../../issues/582) — The extension publish workflow injects `GOOGLE_ANALYTICS_SECRET`, so a CI-published extension stops baking an empty Measurement Protocol secret and sending no events.
- [#571](../../issues/571) — An extension's build hooks run again: the package task resolves the nested `hooks/build/pre.js` the scaffold writes and setup migrates to, keeping the flat `hooks/build:pre.js` as a transition fallback. A miss logs through the framework logger, not an untagged `console.warn`, and the live-reload skip names the mode it skips in.
- [#568](../../issues/568) — The scaffolded web CI workflow stops filling the runner's disk (UJM 1.9.33's fix, ported): `fetch-depth: 1`, `force_orphan: true` on the gh-pages step, `df -h /` before and after the build (after with `if: always()`), and `git ls-files` as the file listing. **The next deploy discards gh-pages history.**
- [#562](../../issues/562) — The /pricing comparison matrix reads a cell the way the plan cards do: a truthy cell draws the check, and anything that is not a bare `true` prints its value beside it. One `features[].value` feeds both surfaces, so `"Included"` stops mixing with x icons in its own row.
- [#563](../../issues/563) — The alternatives hub prints each competitor's authored `description` under its row title, muted and unweighted, above the "View comparison" link. Every `_alternatives` doc writes the key and legacy UJM rendered it; the hub had nowhere for it to land. A competitor without one renders the row unchanged.
- [#561](../../issues/561) — The download and extension shortlinks generate again: `/download/<platform>`, `/download/<platform>/<artifact>` and `/extension/<store>`, one redirect page per entry of the maps `/download` and `/extension` already render from. Legacy parity — a bare platform URL leads with its first artifact. Brands never hand-author these.
- [#580](../../issues/580) — Every section arg naming a literal VALUE escapes on the way out — stat and hero card numbers, timeline years, the hero/cta command, bento's panels, the price card's catalog value — so `<200ms` stops opening a tag the minifier eats. Prose args keep their authored `<em>`.
...
- [#590](../../issues/590) — `omega setup` writes a desktop consumer's `package.json` with npm's own trailing newline, and only when the content changed — the twin of #572. Every build re-stripped that newline, so a consumer's editor or lint hook fought setup on every run.
- [#592](../../issues/592) — All four strings the extension's `config/messages.json` seed renders escape for the single-quoted JSON5 they land in, brand name included. A brand named with an apostrophe seeded a file the build could not parse.
- [#591](../../issues/591) — An extension build hook receives the ONE OMEGA hook argument, `{ manager, projectRoot, mode }` — the shape @omega.js/desktop already passes. The task passed its internal watch counter while both docs described a build-info object nothing ever built. **A hook taking `index` must move to the ctx.**
- [#464](../../issues/464) — Development runs never deliver real ad conversions: the analytics sender carries the same development blocker monitoring has, so emulator seeding cannot reach the live Meta, TikTok, or GA4 endpoints.
- [#465](../../issues/465) — TikTok server conversions send `event_source_id` as the string the Events API expects, ending the 40002 refusals.
- [#469](../../issues/469) — The web build no longer silently demotes a four-segment page module to a helper: the page keeps its JS and CSS.
- [#471](../../issues/471) — Every section declares `headline_accent`, so an accented headline renders whole on bento, product-demo, showcase, timeline, and principles instead of losing its accent silently.
- [#473](../../issues/473) — marketing/cta takes an `enabled` gate honoured by the base index layout, so a page with no CTA copy renders no empty ink panel.
- [#477](../../issues/477) — The pricing monthly equivalent floors (790/12 renders $65), matching the legacy renders instead of drifting $1 up.
- [#497](../../issues/497) — Server conversion payloads carry the page URL (TikTok `page.url`, Meta `event_source_url`) from the attribution touch.
- [#501](../../issues/501) — The legacy `oauth.json` no longer lands inert: the port maps it onto `.omega/secrets/google-oauth.json`, the file the authentication service reads.
- [#502](../../issues/502) — The onboard `.env` stub carries every key the backend disperse composes; five were missing.
- [#503](../../issues/503) — `socials` and `github` join the canonical top-level order, so a config carrying them sorts deterministically.
- [#505](../../issues/505) — The state-retirement migration maps legacy stream keys (`website`) onto real target names instead of writing an invalid `targets.website`.
- [#506](../../issues/506) — Webhook processing trusts only the provider's own lookup; a failed lookup refuses the event instead of falling back to the caller-supplied object.
- [#507](../../issues/507) — The recaptcha service fails the manage walk on a half-keyed pair (`RECAPTCHA_SECRET_KEY` without `siteKey`, or the reverse) instead of passing green while every live checkout 403s.
