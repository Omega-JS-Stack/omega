...
- **C3. Classy full redesign** — ground-up rebuild: modern, clean, light/dark, kickass typography, polished UX; freedom to rip apart, no compat. After C2 so it's built once on the new shape; the omega dogfood site becomes the showcase. **Designed cross-target from day one (D10)** — core omega css/js consumable by desktop/extension too, brand layers on top. **Two-tier consumer story (raw dump 2026-07-11)**: extending a stock theme = main.css-level overrides; authoring a full theme piggybacks the core baked CSS/JS (animations + primitives); license to FULLY BREAK the current theme system granted.
- **C4. Cross-target sharing** — analytics integration module, fontawesome, theme sharing across web/desktop/extension ("brand theme once, all three targets" is the acceptance test); revisit `@omega.js/themes` once C3 stabilizes. **+ forms & ads (raw dump 2026-07-11)**: form-manager.js shared (web-only today), ads as a shared JS module replacing the Jekyll adunit includes — role-keyed `advertising.*` config, ad TYPE param, inhouse-fallback URL (promo-server) de-ITW'd into config.
- **C5. Test-scoping DX (raw dump 2026-07-11)** — bare `npx omega test` from a brand runs that brand's project tests ONLY; `framework:`/`omega:` = framework suites, `full:` = both, `brand:`/`project:` = explicit aliases; folds parked finding 1.2a (backend test path filter).
...
12. ✅ **Provider-discriminated config keys** (added post-decide — Ian 2026-07-10, `# more`): keys that name a ROLE with a `provider` discriminator instead of provider-named top-levels — e.g. `firebaseConfig` becomes `<role>: { provider: 'firebase', … }` — so a consumer could someday switch (supabase etc.) without a config-shape break. NO alternative providers actually built now; shape-only future-proofing. Exact key names land in N4's config review (cheap pre-dogfood, expensive after brands migrate).
    ...
    **D5 addendum 2 (Ian 2026-07-10, REVERSED by Ian same day):** the Cloudflare `omega-api-proxy` worker is KEPT — briefly marked deprecated, then Ian clarified its real purpose: brands whose api domain fronts a DEDICATED non-Firebase backend (proxifly: `api.{domain}` is its own API server; `cloudflare.workers` routes `api.{domain}/backend-manager*` with `FIREBASE_PROJECT_ID` env) need the worker to carve the omega routes out at the Cloudflare edge to the brand's Firebase functions — everything else passes to the origin. Firebase-hosted api domains use hosting rewrites and don't configure it. Deprecation header + manager warning removed; the worker header documents the shape.
...
- **L2. Payment system review** — **upgrades/downgrades (Ian 2026-07-11: CONFIRMED REQUIRED — a subscribed user currently cannot change plans at all)**, post-purchase upsell/add-ons, one-time purchases (known gap), cart, invoice list in /account. **Ecommerce (raw dump 2026-07-11 + Ian's same-day corrections)**: `physical: true` implies a required shipping address and applies to SUBSCRIPTIONS TOO; multi-purchase stays one-time-only (subs never multi-purchasable); mixed checkout allowed — a sub + other items in one cart, but never two subs — and a user with an active sub stays blocked from buying another (upgrade/downgrade is the path instead); cart + checkout flow — the "launch an ecommerce store" story.
...
* so does each brand get a "omega-manager"? or is does only the company get one?
* is whatever systme you designed efficient? or will each brand having an omega manager be a waste of space? maybe its necessary for the lognterm goal of cli/https/cms? idK?
* do we have a CART system? i eventually want to support ecommerce stores where they can checkout and the products should probably have a field that enables them to be multi-purchased (so subscirptions woudl be FALSE for this right?) and then also to require a shipping address like address=true or maybe a physical=true which would inherently require address? idk i want to make an ecommerce store.
* maybe by default, running tests ONLY runs the brand if run from a brand? but user can run framework tests with "npx omega test framework:" or "npx omega test omega:" maybe? or "npx omega test full:" for both framewokr and brand/project? and shoudl we make an alias that does the opposite like "npx omega test brand:" in addition to "npx omega test project:"?
* did you fix it so form-manager.js is shared among all frontend framreworks? what about ads (previously done via jekyll includes --> EWWWW!) needs to be a shared js file. and what abut config for adsense? is it structured via our standards as as something liek advertising.adsense.id = {id} or soemthig simialr instead of toplevel adsense? does our ads js new system support passing a TYPE of ad (or defaukltong to wahtever type is available and then falling back to INHOUSE ads (promo-server.itwcreativeworks.com), which should also be remvoed as hardcoded in our new system and settable as a config)
* theme system: i know we discussed this at length, but I really wanna make it so a consumer can easily do 2 things: The first is to easily extend the existing themes. So for example, let's say they wanna change the colors on the default theme. That should be as easy as doing it in the main.css file (kinda like our current system, but i give you permission to FULLY BREAK THE THEME SYSTEM IN FAVOR OF MAKING IT BETTER AND MORE STREAMLINED). Second, it should be easy to completely define their own theme, allowing it to piggyback off of the core defaults like animations and any other CSS and JS that is baked into the framework.
* is ther eany way we can e2e test a google oauth flow? if not its fine jw.

* translations shoyl dbe settable as a config. web shoudl support it. also specify a provider, iether chatgpt or claude. default c;aude and use the local installed sdk (claude code). and on that note, please enhance the translation system so that its better. the branch caching thing + just the way its transa;ted seems janky. just make it better. test it and confirm it works
* what abt cms? paes, content, plans, config, admin stats etc?

> **Triage (2026-07-11 PM, Claude) — the two bullets above:** translation → **SHIPPED same day (cp96)**: shared `translation` config section (languages/provider/model/exclude), provider `claude` DEFAULT riding the local Claude Code SDK (zero keys; hermetic sessions) with `chatgpt` + OPENAI_API_KEY as the alternative; the branch-cache + gitignored-cache jank replaced by a COMMITTED per-string cache (`translations/`); web fully supported (build hook + real `omega translate`); live-tested on both surfaces — docs/translation.md. CMS → answered in-chat: that's the **L3 arc** (git-backed CMS — pages/content/plans/config editing + admin stats — served whitelabel from /admin on the D9 hosted-company + D13 one-executor substrate); sequenced after the dogfood arc closes; cp98 D13 builds the deploy layer it rides on.

> **Triage (2026-07-11, Claude) — the seven bullets above, routed:**
> 1. **Manager per brand? Answered — ONE install per company, by design.** Company mode spawns each brand's manage child on the COMPANY install's own bin (cwd = brand root), so brands under a company carry no manager of their own; a standalone brand rides `npx omega-manager` (or a devDep). No per-brand disk waste — and the company-root install is exactly the thing D9's hosted-company-omega runs server-side, so the shape already serves the CLI/HTTP/CMS long game (your hunch was right).
> 2. **Cart/ecommerce → L2 enriched**: no cart today (known gap, already L2); adds product fields for multi-purchasable one-time items (`physical: true` implying a required shipping address), cart + checkout flow. **Ian corrections (same day)**: `physical` applies to subs too; mixed cart OK (one sub + other items, never two subs; active-sub users still blocked from a second sub); **upgrade/downgrade CONFIRMED REQUIRED — does not exist today**.
> 3. **Test-scoping DX → NEW C5** (adopting the proposed syntax): bare `npx omega test` from a brand = that brand's project tests ONLY; `framework:`/`omega:` runs the framework suites, `full:` runs both, `brand:`/`project:` = explicit aliases of the default. Touches the backend runner's path filter — parked finding 1.2a folds in.
> 4. **Forms + ads → C4 enriched (survey 2026-07-11 confirms the gap)**: `form-manager.js` lives in web/core ONLY — not shared with desktop/extension; ads are still Jekyll includes (`modules/adunits/adsense.html` + `promo-server.html`) driving `vert.js`, which HARDCODES `https://promo-server.itwcreativeworks.com` (origin check + baseURL) and a `brand.id === 'promo-server'` dev check. C4 gains: shared form + ads JS modules, role-keyed `advertising.*` config (`advertising.adsense.id`-style per the D12 principle, not a top-level `adsense`), an ad TYPE param with available-type default, and the inhouse fallback URL de-ITW'd into config. **Ian confirmed (same day): queued with C4, not now.**
> 5. **Theme two-tier consumer story → C3 acceptance criteria**: (a) extending a stock theme = main.css-level overrides, nothing more; (b) authoring a full theme piggybacks the core baked CSS/JS (animations + primitives). License to FULLY BREAK the current theme system recorded in C3. **Ian confirmed (same day): queued with C3, not now.**
> 6. **OAuth e2e? Answered — emulator yes, live Google no.** The Auth emulator fakes the Google IdP end-to-end (its own account-chooser page, zero real Google), so a sign-in-with-Google browser e2e is buildable on the N6 harness whenever a consumer needs it (queued LATER as a harness extension); a REAL Google flow stays untestable by design — bot detection, 2FA, and ToS all fight automation.

> **Triage (2026-07-10): already the design — no new decision needed.** The brand monorepo IS the SSOT (its `config/omega.json5` is the one file the manager AND the frameworks read — no `.brands/` mirror, nothing dispersed); the company root is the SSOT for company-wide values, consumed by RESOLUTION not copying (config merge layer `defaults ← company ← brand ← app`, D15 env cascade, brands point at the company via the `.omega/company.json` marker); disperse only materializes deploy-artifact necessities (backend `functions/.env`, GA4 stream secrets, cert paths). Company-level ASSETS (PSD templates → the company repo) get their real test at the ITW dogfood migration — any surviving copy-flow gets killed there.
