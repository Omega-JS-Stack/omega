...
- **L2. Payment system review** — upgrades/downgrades, post-purchase upsell/add-ons, one-time purchases (known gap), cart, invoice list in /account. **Ecommerce (raw dump 2026-07-11)**: product fields for multi-purchasable one-time items (`physical: true` implying a required shipping address; subscriptions excluded from multi-purchase), cart + checkout flow — the "launch an ecommerce store" story.
...
> 2. **Cart/ecommerce → L2 enriched**: no cart today (known gap, already L2); adds product fields for multi-purchasable one-time items (`physical: true` implying a required shipping address; subscriptions excluded from multi-purchase), cart + checkout flow.
...
> 4. **Forms + ads → C4 enriched (survey 2026-07-11 confirms the gap)**: `form-manager.js` lives in web/core ONLY — not shared with desktop/extension; ads are still Jekyll includes (`modules/adunits/adsense.html` + `promo-server.html`) driving `vert.js`, which HARDCODES `https://promo-server.itwcreativeworks.com` (origin check + baseURL) and a `brand.id === 'promo-server'` dev check. C4 gains: shared form + ads JS modules, role-keyed `advertising.*` config (`advertising.adsense.id`-style per the D12 principle, not a top-level `adsense`), an ad TYPE param with available-type default, and the inhouse fallback URL de-ITW'd into config.
> 5. **Theme two-tier consumer story → C3 acceptance criteria**: (a) extending a stock theme = main.css-level overrides, nothing more; (b) authoring a full theme piggybacks the core baked CSS/JS (animations + primitives). License to FULLY BREAK the current theme system recorded in C3.
