...
> - **`min || 0` — FIXED cp84** (Ian: "min should not be enforced if not provided. Same as max."). Bounds enforce ONLY when declared: undeclared `min` lets negatives through to handlers; declared `min: 0`/`max: 0` are real bounds (they used to vanish via `||`). Literal pins in the parity suite (`min-max-enforce-only-when-declared`); corpus green — no framework schema relied on the implicit floor.
