...
## 🎯 Now
- **SECTIONS SPEC DRAFTED (cp202) — awaiting Ian's ratification**: [plans/omega-sections-spec.md](plans/omega-sections-spec.md) consolidates the 2026-07-18 design night — section/component library (two tiers, nested ids, consumer-local layer, YAML block args), minimal frontmatter, `[id].js` kills asset_path, blueprints/absence stays + `omega customize` prefill, post-build translation stays, Liquid stays, web engine = the one HTML factory for ALL frameworks; ratify → extraction begins
