...
## 🎯 Now
- **SECTIONS SPEC RATIFIED (cp202/203, Ian 2026-07-18) — extraction IN FLIGHT, cp204–207 LANDED**: [plans/omega-sections-spec.md](plans/omega-sections-spec.md) is binding; cp204 = machinery + hero; cp205 = whole classy index is composition (testimonials shared ×4, shared-band ruling → spec §6); cp206 = CTA-family convergence (one union section serves 8 pages, ~170 dup lines die); cp207 = newsletter-cta extracted (form-manager dialect canonical; blog/post's DEAD 404-form parked for the §7 JS lane) — goldens all zero-content-diff; web 201/0, corpus 7/7; next: **spec §7 asset lanes (section scss/js)** → remaining classy pages → newsflash
...
- **blog/post's newsletter form is DEAD (cp207 find)**: it posts `action="/email-subscription"` — a page that does not exist (404 on submit); the canonical dialect is the client form-manager one (blog/index, now `marketing/newsletter-cta`) but its `#newsletter-form` binding lives in PAGE js (`core/js/pages/blog/index.js`) — post's conversion lands with the spec §7 section-JS lane (DOM-presence init), which makes the section own its binding on any page
