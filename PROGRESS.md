...
## 🎯 Now
- **Parked-findings sweep LANDED (cp198)**: FOUC killed (theme-aware font preloads), permalinkOf regex fixed, SW cached-404 closed (no fetch handler), sandbox e2e promoted to root `npm test` — web 176/0, corpus 7/7, all workspaces green
...
- ~~permalinkOf regex (155)~~ **FIXED cp198**: regex widened from `\S+` to `.+?` with trim; 10-test suite pinned
...
- [x] 198 parked-findings sweep — FOUC killed (theme-aware font preloads, both themes pinned, corpus invariant) · permalinkOf regex fixed (10-test pin) · SW cached-404 CLOSED (no fetch handler) · sandbox e2e promoted to root `npm test` (`OMEGA_SKIP_E2E=1` knob); web 176/0, corpus 7/7, sandbox 1242/0 → CHANGELOG
...
*Last updated: 2026-07-17 (cp198: parked-findings sweep — FOUC fixed, permalinkOf fixed, SW closed, test:e2e promoted)*
