/**
 * Redirect layout module (`js/layouts/modules/utilities/redirect.js`, #624) —
 * the JS half of the `modules/utilities/redirect` layout, loaded on every page
 * that layout renders and on nothing else.
 *
 ...
 *
 * It hops at IMPORT time rather than from a default export: a shortlink must
 * not wait on the @omega.js/client boot the page's other bundles await, and the
 * head's `<noscript>` meta-refresh is the no-JS half this must keep beating.
 */
