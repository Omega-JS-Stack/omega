...
export function buildBindingsState() {
  ...
  return {
    ...
    // The live User, never a copy: `auth` is one bindings key, so a plain
    // object here would replace the User every other `auth.user.*` binding reads
    auth: {
      user: omega.auth.user,
    },
  };
}
