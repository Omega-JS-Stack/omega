...
/**
 * Resolve the @omegajs/client entry (the `web-manager` esbuild alias target).
...
function resolveClientEntry() {
  try {
    return require.resolve('web-manager');
  } catch (error) {
    throw new Error('Could not resolve the web-manager client package — reinstall @omegajs/web (npu install) and retry');
  }
}
