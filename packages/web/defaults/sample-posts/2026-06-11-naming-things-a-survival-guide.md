...
A function is a promise to do something — name the promise. `sendReceipt` beats `receipt`. And when a function's honest name turns out to be `sendReceiptAndUpdateLedgerAndNotifySlack`, the name has done its real job: it told you the function needs to be three functions.
