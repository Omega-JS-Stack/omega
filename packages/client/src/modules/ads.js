/**
 * Ads module — the fallback-ladder ad engine shared by every surface
 ...
 *      in-house source's /omega/ads/serve, origin-validated postMessage with
 *      a fixed vocabulary (omega-ad:set-dimensions / omega-ad:click), and
 ...
 *      itself and emits omega-ad:no-fill). The iframe only renders and
 ...
 * data-omega-ad* attributes; bind(root) scans [data-omega-ad] and mounts
 * every match. The web ads/unit section delegates here, and desktop/
...
const MESSAGE_DIMENSIONS = 'omega-ad:set-dimensions';
const MESSAGE_CLICK = 'omega-ad:click';
...
class AdUnit {
   ...
   * @param {string} [options.adId] - pin a specific ad
   ...
   */
  buildServeUrl() {
    const url = new URL(`${this.source}/omega/ads/serve`);
    ...
    if (this.options.adId) {
      url.searchParams.set('adId', this.options.adId);
    }
  ...
  _emit(name, detail) {
    ...
    if (typeof callback === 'function') {
      try {
        ...
        console.error(`[Ads] on${name} callback error:`, e);
      }
    }

    if (typeof CustomEvent !== 'undefined' && typeof this.$el.dispatchEvent === 'function') {
      this.$el.dispatchEvent(new CustomEvent(`omega-ad:${name}`, { detail, bubbles: true }));
    }
  }
}
...
class Ads {
  ...
  resolveSource(source) {
    ...
    if (configured === 'company') {
      ...
      if (!companyUrl) {
        console.warn('[Ads] inhouse source is "company" but config.company.url is not set');
        return null;
      }
      return this.manager.getApiUrl(null, companyUrl);
    }

    console.warn('[Ads] Unsupported inhouse source:', configured);
    return null;
  }

  /**
   * Read a host element's data-omega-ad* attributes into render options.
   * @param {Element} $el - element carrying the data-omega-ad vocabulary
   * @returns {object} { type, size, adId, tags }
   */
  parseElementOptions($el) {
    const attr = (name) => (typeof $el.getAttribute === 'function' && $el.getAttribute(name)) || '';

    return {
      type: attr('data-omega-ad') || 'display',
      size: attr('data-omega-ad-size'),
      adId: attr('data-omega-ad-id'),
      tags: attr('data-omega-ad-tags')
        .split(',')
        .map((tag) => tag.trim())
        .filter(Boolean),
    };
   ...
   * read from the element's data-omega-ad* attributes; passed options win.
   ...
   */
  mount($el, options = {}) {
    if (!$el || $el.__omegaAdMounted) {
      return null;
    }
    $el.__omegaAdMounted = true;
  ...
  /**
   * Auto-bind every [data-omega-ad] element under a root — the surface hook
  ...
  bind(root) {
    ...
    const mounted = [];
    scope.querySelectorAll('[data-omega-ad]').forEach(($el) => {
      if ($el.__omegaAdMounted) {
        return;
      }
      this.mount($el);
      mounted.push($el);
   ...
   * no-fill collapse.
   * @param {Element} $el - host element
   * @param {object} [options] - AdUnit options + { type }
   * @returns {Promise<{ lane: string, unit?: AdUnit }|null>}
   */
  async render($el, options = {}) {
    ...
    if (!format) {
      console.warn('[Ads] Unsupported ad type:', type);
    ...
    try {
      ...
      console.warn('[Ads] AdSense script blocked/failed — fallback lane:', e?.message || e);
      return this._fallback($el, options);
    }
   ...
   * @param {Element} $el - host element
   * @param {object} [options] - AdUnit options ({ source } overrides config)
   * @returns {{ lane: string, unit: AdUnit }|null} null when no source resolves
   */
  renderHouse($el, options = {}) {
    ...
    const unit = new AdUnit(this.manager, $el, { ...options, source, tags }).load();
  ...
  _emitHost($el, options, name, detail) {
    ...
    if (typeof CustomEvent !== 'undefined' && typeof $el.dispatchEvent === 'function') {
      $el.dispatchEvent(new CustomEvent(`omega-ad:${name}`, { detail, bubbles: true }));
    }
  }
