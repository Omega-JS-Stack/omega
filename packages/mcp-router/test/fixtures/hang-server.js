#!/usr/bin/env node
/**
 * A BROKEN upstream for the spawn-deadline test: it writes garbage to stdout
 * and never answers initialize, so a client.connect() against it never settles
 * on its own. That is the shape that used to wedge an upstream for a whole
 * session.
 *
