...
| `src/merge-line-files.js` | `mergeLineBasedFiles(existing, incoming, fileName)` — the OMEGA marker-section merge (.env/.gitignore/CLAUDE.md: framework owns `Default Values`, consumer owns `Custom Values`; .env values normalize to double-quoted) | EM's evolved copy (canonical: quote normalization + order-safe key substitution); BXM/UJM carried older inline variants |
