...
7. Read the two allow lists the install wrote, and narrow them if you want to ([#875](https://github.com/Omega-JS-Stack/omega/issues/875), § The job guard below): `%LOCALAPPDATA%\omega-runner\allowed-repos.txt` and `allowed-actors.txt`. The install fills them with `<org>/*` for every org it registered and with your `GH_TOKEN` user's login; both are plain text, one entry per line, `#` comments allowed, and nothing rewrites a line you changed.
...
The box holds the EV token, so whoever can start a job on it can sign binaries with your cert. The workflows this framework generates fire only on a dispatch, but a workflow is one careless edit away from a `push` trigger, so the box refuses the job itself ([#875](https://github.com/Omega-JS-Stack/omega/issues/875)).

`npx omega runner install` (and `start` / `restart`, which heal a box installed before the guard existed) writes three files:

| File | What it is |
|---|---|
| `%LOCALAPPDATA%\omega-runner\job-started.js` | the hook, a copy of the framework's own, rewritten from the package on every install and start so an update reaches the box |
| `%LOCALAPPDATA%\omega-runner\allowed-repos.txt` | one `owner/name` per line; `owner/*` means every repo of one owner |
| `%LOCALAPPDATA%\omega-runner\allowed-actors.txt` | one GitHub login per line |

and delivers the hook's path to each registration through that runner's own `<runner dir>\.env` as `ACTIONS_RUNNER_HOOK_JOB_STARTED`: the same file the private HOME travels in, so one write reaches the Startup shortcut and the detached spawn alike. GitHub's runner executes it before a job's first step and FAILS the job on a nonzero exit, with the hook's output in the job's `Set up runner` step. The runner runs it with its own bundled node; nothing on your PATH is involved.

Three checks, all of which must pass:

| Check | Refused when |
|---|---|
| `GITHUB_EVENT_NAME` | it is not `workflow_dispatch`, the ONE trusted event (#923): every publish path (a CMS button, `omega deploy`, a plain HTTP call) dispatches it at the deploy ref |
| `GITHUB_REPOSITORY` | no line of `allowed-repos.txt` matches it |
| `GITHUB_ACTOR` | no line of `allowed-actors.txt` matches it. A dispatch carries the deploy token's user, which is the same account |

A miss prints ONE line naming the check and the value it saw, and the job dies with nothing run: no checkout, no install, no `signtool`.

**Editing the lists is the point.** They start as the scope the registration already has: an `<org>/*` line per registered org, and the `GH_TOKEN` user's login. Narrow `Omega-JS-Stack/*` to `Omega-JS-Stack/my-brand-omega` and the next `install` leaves it alone: an org that already has ANY line is never appended to again. Add a second account by adding a line. An EMPTY list refuses every job rather than passing one, which is what a box whose `GH_TOKEN` could not answer at install time will do until you fill `allowed-actors.txt` in.

`uninstall` keeps both lists (they are the box's configuration, like its `.env`) and removes the hook script, which the next install writes back.
...
**The runner's own `<runner dir>\.env`** carries two framework-written lines, `HOME` (below) and `ACTIONS_RUNNER_HOOK_JOB_STARTED` (§ The job guard). It is the file GitHub documents for runner env, one `KEY=value` per line, read by the listener at startup and applied to every job it runs; a change to it takes effect at the next `npx omega runner restart`.
