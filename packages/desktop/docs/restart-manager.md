...
**Protocol v2 rewrite** — the legacy `restart-manager://message?command=register&payload=<json>` deep-link handshake is GONE (it was fire-and-forget with no ack, and RM polled process NAMES). RM now serves a loopback HTTP protocol and monitors exact PIDs. The RM app itself is an EM consumer at `restart-manager/restart-manager-desktop` and is the serving side of everything below.
...
Neutral ground, deliberately NOT any app's userData (EM's dev/test userData suffixing never moves it): `resolveSharedRoot(appDataPath, env)` → `env.EM_RM_ROOT || <appData>/restart-manager/`.
...
└── install.lock      ← advisory lock ({ pid, hostAppId, acquiredAt }, stale after 10 min) so two
                        EM apps can't run installers concurrently
```

`EM_RM_ROOT` is the cross-repo test/dev isolation seam. In EM's own test mode the lib roots at `<userData>/restart-manager` (the ` (Testing)` dir, wiped per run) — tests never touch the real root.
...
1. Fetch the release feed `https://github.com/<owner>/<repo>/releases/latest/download/<latest-mac.yml | latest.yml | latest-linux.yml>` (js-yaml). Default feed: **`restart-manager/update-server`** — RM publishes through EM's standard release pipeline, so the feed files + versioned artifacts are the normal ones.
...
Failures set a 1-hour install cooldown (no download storms); the advisory `install.lock` serializes concurrent installers across EM apps.

## Updates — RM updates ITSELF (EM's standard autoUpdater)

This lib only ever **installs** RM when missing. Once running, RM keeps itself fresh exactly like every other EM app — `autoUpdate.enabled: true`, standard electron-updater flow (NSIS on win, zip-in-place on mac, AppImage on linux), applying on the user-idle install policy with the 30d force gate. No bespoke update logic anywhere.

The update relaunch is safe by construction: RM's registrations are **storage-persisted** (its boot prune keeps live-pid entries), watched apps' 60s heartbeats reconnect through the freshly written runtime.json, and EM's single-instance lock quits any duplicate spawned during the handover window.
...
# 2. Any EM consumer registers against it
...
# 5. open "restart-manager://app/show" → RM's dashboard (EM built-in route)
```
