...
Both @omega.js/desktop test runners (`runners/electron.js`, `runners/boot.js`) set `OMEGA_TEST_MODE=true` in the spawned child env. That powers `manager.isTesting()` (and `Manager.isTesting()` static) — the cross-context helper everything in @omega.js/desktop checks when it needs to behave differently in tests:
...
Consumers writing their own tests should set `OMEGA_TEST_MODE=true` in their test runner so the same signal applies — for example, in `package.json`:
```json
