...
Convention-only. No config block — drop PNGs at known paths and @omegajs/desktop finds them.
...
config/icons/
  ...
  macos/              ← macOS overrides (beats global)
    icon.png
    tray.png          ← 32×32 native; @omegajs/desktop renames to trayTemplate.png in dist
...
5. `<@omegajs/desktop>/dist/config/icons/windows/<file>` — Linux-only extra step at the bundled level
...
Retina slots (macOS tray, macOS dmg) take ONE source file at the native (@2x) size. @omegajs/desktop downscales the @1x sibling at build time via `sharp` and writes both into `dist/`. Consumers never ship `<name>@2x.png` files.

| Slot | Native size | @omegajs/desktop emits |
|---|---|---|
...
macOS reads the literal filename of the tray icon and treats any `*Template.png` as a "template image" — pure-black with alpha, automatically inverted in dark mode. Consumers ship `tray.png` (clearer naming, matches Windows/Linux); @omegajs/desktop owns the `Template` suffix when writing to `dist/`.

If you set a custom path via `tray.icon(path)` in your `src/integrations/tray/index.js`, YOU are responsible for the runtime filename containing `Template` — @omegajs/desktop only owns the convention path.
...
@omegajs/desktop ships its own `icon.png`, `tray.png`, `dmg.png` for each platform in `<@omegajs/desktop>/src/defaults/config/icons/<platform>/`. These are the final fallback when neither the consumer nor a global file provides anything — so a fresh `npx mgr setup` project produces a buildable app with the generic @omegajs/desktop icon out of the box.
