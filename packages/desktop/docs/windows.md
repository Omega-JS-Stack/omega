...
The page template (`<em>/src/config/page-template.html`, framework-internal — not consumer-overrideable) ships an `.em-titlebar` div with `-webkit-app-region: drag`. Per-platform spacing is handled by `themes/classy/css/components/_titlebar.scss`, which keys off `html[data-platform]` (set by @omegajs/client during init):
