...
- **Config format** — `config/omega.json5` (the single OMEGA config, JSON5), resolved via the vendored `@omega.js/config`: shared sections top-level, desktop settings under `targets.desktop` (overlaid onto the top level at load). The legacy `config/electron-manager.json` is no longer read — migration table in the monorepo's `docs/shared/config.md`.
...
### ✅ 2026-07-03 — dockless hidden login launches + classy $min-contrast-ratio (Unreleased)
- [x] `build-config` injects `mac.extendInfo.LSUIElement: true` when `startup.openAtLogin.mode === 'hidden'` too (not just `startup.mode: 'hidden'`) — a mode-normal app launched hidden at login FLASHED the dock (the bounce is native + pre-JS; `applyEarly()`'s `dock.hide()` was always too late). Manual launches start dockless; the dock icon appears when a window surfaces (every surface path already runs `_ensureDockVisible()` → `dock.show()`). Extracted + exported pure `shouldInjectLSUIElement(config)`; build-config suite +1 (25/25). Docs: startup.md zero-bounce section; CHANGELOG Unreleased.
- [x] Classy theme: `$min-contrast-ratio` declared `!default` + forwarded into the Bootstrap `with()` — consumers with light brand colors keep white button/badge text (Somiibo: `$primary: #1FB7F2, $min-contrast-ratio: 2` — verified in its compiled bundle: `.btn-primary` bg `#1FB7F2`, color `#fff`). CHANGELOG Unreleased.
- [x] Consumer verification: Somiibo build log shows `startup.openAtLogin.mode=hidden → injected mac.extendInfo.LSUIElement=true`; its full project suite green with the linked EM. UNCOMMITTED — rides the next EM ship.

### ✅ 2026-07-03 — v1.12.0: session persistence + renderer WM cycle + real plan in main

- [x] `lib/auth-persistence.js` — pluggable main-session vault (Ian: adaptable, storage-only — distribution stays IPC): strategy interface `{ available, getItem, setItem, removeItem }` + `register()` for custom ones; `safeStorage` default (OS-keychain encryption, ciphertext-only file `{userData}/em-auth-session.json` — STRONGER than browser storage: IndexedDB/localStorage are plaintext LevelDB, only cookies get os_crypt; Ian's "no raw JSON" requirement met the right way); `none` opt-out; corrupt/undecryptable/unavailable → signed-out + logged, never a crash. Bridge boots via `initializeAuth(app, { persistence })` (the `getReactNativePersistence` adapter shape).
- [x] EM renderer now runs WM's FULL auth cycle (`auth().listen()` persistent) — Firestore account fetch + `resolveSubscription` + **`data-wm-bind` bindings auto-populate in every renderer** (the UJM/BXM pattern Ian pointed at; BXM docs/auth.md = the reference). Each settle pushes `{ uid, resolved, roles }` → `em:auth:account-resolved` (uid-guarded; re-offered on `em:auth:state-changed` — closes the renderer-resolved-before-main-signed-in race found live); bridge caches → `getResolvedPlan()`/`getResolvedRoles()` + `em:auth:plan-changed` broadcast + sign-out clear. Fixes Somiibo's wrong-plan dropdown: plan-enforcer read a NEVER-populated `_resolvedPlan` → basic forever.
- [x] Tests: new main/auth-persistence ×5 (registry + custom strategy, adapter round-trip, REAL safeStorage encrypt/decrypt + garbage tolerance, uid-guarded intake w/ broadcast-once + sign-out clear); full suite 750/5 green. LIVE-verified in somiibo-desktop-2, all three paths: restart → `auth state → ian…` restored from safeStorage in ~400ms + `account resolved — plan=pro` (his real plan — was 'basic' before); mid-session sign-out→sign-in → plan re-lands in <1s; `somiibo:plan:get-limits` flips basic→pro (8 modules/∞ runtime/5 sessions). Docs: web-manager-bridge.md §§ persistence/API/renderer-cycle, CLAUDE.md lib rows + docs index, CHANGELOG 1.12.0.

### ✅ 2026-07-03 — v1.11.1: bridge firebase bundled (dev no-op bug, found live)

- [x] The first live dev sign-in round trip exposed it: `handleAuthToken called but firebase not loaded — ignoring`. Root cause: the bridge's `webpackIgnore`'d runtime `import('firebase/app')` resolves relative to the CONSUMER's `main.bundle.js` — the walk never reaches EM's node_modules under a symlinked EM (`mgr install dev`) and rides npm hoisting when installed. Fix: plain `require()` so webpack BUNDLES firebase from EM's module context (json5 precedent; no externals, sharedResolve covers it), interop guards for both namespace shapes, and the load-failure reason is logged instead of swallowed. Bridge suite 14 + full suite 745/5 green; verified live in somiibo-desktop-2 — `firebase loaded` at boot, then the WHOLE openAuthFlow round trip completed hands-free (browser SSO bounce → loopback → `auth state → ian.wiedenman@gmail.com`).

### ✅ 2026-07-03 — openAuthFlow(): EM-owned sign-in launch + dev loopback return (RFC 8252)

- [x] New `lib/auth-flow.js` + `manager.openAuthFlow()` (prototype delegate; lib exposed as `manager.authFlow`, initialized at boot step 6b). ALWAYS the user's real default browser (Ian: embedded sign-in is banned — SSO with their own session). Production: plain external open of `getAuthUrl()`, OS scheme returns. Dev/test: the scheme isn't OS-registered (protocol.js is production-only; macOS can't runtime-register unlisted schemes), so the final hop becomes a ONE-SHOT loopback listener — ephemeral 127.0.0.1 port, `state` nonce, 5-min self-expiry (injectable `timeoutMs`), bad state/missing token → 400 while the listener SURVIVES for the real return, success → 200 page + `deepLink.dispatch('<brand.id>://auth/token?authToken=…')` (byte-identical downstream) + stealth-aware `windows.show('main')`. Teardown is real: `Connection: close` on every response + `closeIdleConnections()` in `cancel()` (Node's keep-alive agent otherwise keeps serving a closed listener — caught by the suite).
- [x] `getAuthUrl(env, returnUrl)` — optional final-hop override feeding the above; default unchanged. +1 url-helpers test (17).
- [x] New `main/auth-flow` suite (7 tests): real listener + real HTTP GETs simulate the browser return; stubs only `shell.openExternal`/`handleAuthToken`/`windows.show`. Paired WM 4.3.4 change: `isValidRedirectUrl` accepts loopback hosts (any port) while the SITE runs in dev (its token page otherwise rejects the return URL). Consumer: somiibo-desktop-2 sign-in → `manager.openAuthFlow()`; its shell-ui boot E2E now walks the WHOLE dev round trip (click → chain decompose → real GET with the nonce → `handleAuthToken` receives the token → listener gone). Full mgr + project sweep 1446 passing / 10 skipped. Docs: environment-detection.md § URL helpers, deep-link.md builtin row, CLAUDE.md cross-context helpers, CHANGELOG Unreleased.

### ✅ 2026-07-03 — getAuthUrl() sign-in round-trip + auth/token accepts the website's real token shapes

- [x] `utils/url-helpers.js`: new `getAuthUrl(environment)` — `<site>/signin?authReturnUrl=<site>/token?authReturnUrl=<brand.id>://auth/token` (site = `getWebsiteUrl()`; throws without `brand.id`). Built for Somiibo's account-dropdown Sign in: the bare `/signin` link stranded the login in the browser; this chain makes UJM mint a custom token (BEM `/backend-manager/user/token`) and deep-link it back into the app. Attached in all four Manager contexts + module export. 3 tests in main/url-helpers (decompose the URL with the same searchParams parsing the website performs — no string-matching encodings). Docs: environment-detection.md § URL helpers, CLAUDE.md cross-context helpers.
- [x] `lib/deep-link.js` auth/token builtin: reads ONLY `?authToken=` (Ian's call — ONE modern shape; was `?token=`, which no website flow ever sent). Legacy formats are UJM's concern: its token page's `?payload=` wrap went ADDITIVE (one-line paired UJM edit — the `delete('authToken')` removed; custom-scheme redirects carry both params, each consumer reads its own). EM test asserts `?token=`/`?payload=` are IGNORED; web-manager-bridge's wiring test moved to `?authToken=` too. Docs: deep-link.md builtin-routes table.
- [x] Consumer wired: somiibo-desktop-2's sign-in opens `manager.getAuthUrl()` (its shell-ui boot E2E stubs `shell.openExternal` and drives the real overlay action IPC). Full mgr + project suite green (1439 passing).

### ✅ 2026-07-03 — dock auto-rehide for hidden-mode apps (window-manager)

- [x] `lib/window-manager.js`: new `_maybeRehideDock()` (inverse of `_ensureDockVisible`) wired to every named window's `hide` event — for `startup.mode: 'hidden'` (LSUIElement) apps, hiding the LAST visible window (hide-on-close X / `windows.hide()` / raw `win.hide()`) calls `app.dock.hide()` so the app fully disappears again (dock + Cmd+Tab). Guards: darwin-only, hidden-mode-only, `_isQuitting` skip, any-window-still-visible skip, dock-already-hidden skip. Motivation: closing restart-manager's dashboard stranded its dock icon forever.
- [x] Every sanctioned re-surface path restores the dock (`windows.show()`/deep-link `app/show`/macOS `activate` → `_ensureDockVisible`). RM boot test covers the red-X path end-to-end (close → hidden, not destroyed, server alive; dock state itself is unobservable under test stealth — verified live). Docs: windows.md "macOS dock auto-show" + startup.md hidden-mode section; CHANGELOG Unreleased.
...
- [x] `lib/storage.js`: replaced the `webpackIgnore`'d `importESM('electron-store')` with a static-specifier `import(/* webpackMode: "eager" */ 'electron-store')` — webpack now inlines electron-store into the consumer's `main.bundle.js`. Root cause of the packaged no-op: EM is a devDependency (never in the asar), so the runtime import found nothing; consumers had to install electron-store themselves (the RM/Somiibo workaround). Deleted the now-orphaned `utils/import-esm.js` (storage.js was its only consumer).
- [x] New boot regression suite `suites/boot/storage-bundled.test.js`: (1) no live `import('electron-store')` survives in the built fixture bundle — THE assertion that fails on the webpackIgnore pattern (the functional test alone false-passes in-repo via the upward node_modules walk); (2) real store round-trip + on-disk `em-storage.json` through the bundle.
- [x] Verified: full EM suite 732 passing / 0 failing; RM consumer with electron-store UNINSTALLED → 60/60 green, packaged asar has ZERO node_modules, live packaged smoke: register → graceful TERM → relaunch → registration persisted (`em-storage.json` real, not no-op).
- [x] Docs: `docs/storage.md` implementation note rewritten (was claiming `Manager.require()` + electron-store@10), `docs/boot-sequence.md` step 4, CLAUDE.md File Conventions (importESM → eager-import rule, "NEVER webpackIgnore a dep import"), stale comment examples in `test/runners/boot.js` + `docs/test-boot-layer.md`, CHANGELOG Unreleased section.
- [x] Consumers can now DROP `electron-store` from dependencies (restart-manager-desktop done in the same pass; somiibo-desktop-2 still carries it — remove when it picks up this EM).
- [x] SHIPPED as v1.10.0 (npm + GH release, 2026-07-02) — one release carrying all three unreleased bodies of work: RM protocol v2, FontAwesome/tooltips, this storage fix.
...
- [x] `docs/cdp-debugging.md`: Chrome section rewritten for the per-session isolated browser model (mirrored across UJM/BEM/BXM/EM/WM — Claude sessions auto-launch their own private Chrome via the `chrome-devtools` MCP; no 9223/agent-profile/`CHROME_CDP_PORT` workflow). Electron sections (`mgr cdp`, `chrome-devtools-electron` attach-by-port) unchanged; fixed stale upstream config path. CHANGELOG [Unreleased] entry added. Uncommitted — ship with the next EM release.
- [x] OMEGA mirror mandate: Doc-update parity documents the mirrored structure (canonical skeletons: omega:main mirror-spec); maintainer mirror note in src/defaults/CLAUDE.md + `Per-process imports` → canonical `Per-context imports`.
...
### ✅ Pass 2.26 — Restart Manager protocol v2 (DONE, 713 passing framework-wide)

Full rewrite of `lib/restart-manager` — the legacy `restart-manager://` deep-link handshake is gone; the guardian now serves loopback HTTP with PID-exact monitoring. Built in lockstep with the brand-new RM app (`Restart-Manager/restart-manager-desktop`, an EM consumer — 61 tests of its own + packaged smoke incl. a live crash→relaunch).

- Split dir `src/lib/restart-manager/{index,protocol,install}.js` — `protocol.js` is the wire-contract SSOT the RM app imports via the new `exports` entries (`electron-manager/lib/restart-manager/protocol`). Shared root `<appData>/restart-manager/` (runtime.json + app/ + install.lock), `EM_RM_ROOT` isolation seam.
- Register lifecycle: probe → (install+spawn) → POST register w/ own pid; 60s idempotent heartbeat (revives dead RM); preventDefault-once + 1s-capped deregister flush on quit (quitAndInstall path never intercepted).
- Silent install w/ smart existence: mac zip (unchanged) / **win NSIS run with `/S`** (fully silent, per-user — and self-updatable by electron-updater) / **linux AppImage, browser-.deb dropped**; feed discovery via `releases/latest/download/latest*.yml` (default `restart-manager/update-server`, config-overridable); atomic app.tmp swap (mac/linux); advisory lock; 1h cooldown. **RM self-updates via EM's standard autoUpdater** — the lib only installs when missing (registrations persist RM-side across its update relaunch). No /v1/quit endpoint, no installed.json — deleted with the EM-owned-update design.
- Bonus framework fix: `lib/protocol` now actually QUITS a duplicate instance on lost single-instance lock (was warn-only despite the comment saying "exit immediately").
- Config: `restartManager.feed.{owner,repo,url}` schema entries; old `{ enabled: true }` consumers need zero changes.
- Tests: 2 new build suites (protocol SSOT, install machinery), main suite rewritten against a REAL fixture server implementing the contract, boot suite (bundling), extended live-feed test (skips until first RM release). Docs: restart-manager.md rewritten (protocol reference, threat model, dev loop), lib-modules split note, boot-sequence 13e, CLAUDE.md lib line, installer-options array-replace fix.
- Ian-gated leftovers: create the GitHub repos (`restart-manager/restart-manager-desktop` + org update-server/download-server), first `npm run release`, then `TEST_EXTENDED_MODE=true` against the live feed.

### ✅ Pass 2.27 — Bundled FontAwesome + zero-setup Bootstrap tooltips (DONE, 730 passing framework-wide, committed `619c535`)

Built for Somiibo's "icons on every button / stop reinventing tooltips" batch — both features are consumer-zero-setup.

- **FontAwesome ships in EM**: FA Pro solid (4,799) + brands (609) SVGs vendored at `src/assets/icons/font-awesome/` (mirrors UJM's set + update process — see docs/fontawesome.md incl. license note). New `lib/fontawesome.js` (boot 3c): `manager.fontawesome.get(name, style)` → svg|null (slug-sanitized, cached, `em:fontawesome:get` IPC); `window.em.fontawesome.get` preload bridge; renderer `_wireFontAwesome` auto-injects the SVG into any `<i class="fa-solid fa-*">` (1em/currentColor, MutationObserver-live, modifier classes never mistaken for names). **Gotcha fixed after live consumer testing**: a consumer's webpack-BUNDLED main loses the lib's real `__dirname` — `_resolveRoot()` falls back to `<appPath>/node_modules/electron-manager/dist/assets/...` (valid in dev and inside app.asar). Core scss `core/_fontawesome.scss` handles display/baseline.
- **Bootstrap JS ships in EM**: prebuilt UMD of the vendored Bootstrap 5.3.6 source with Popper inlined (`assets/themes/bootstrap/js/bootstrap.bundle.js`; `@popperjs/core` added as a dep for rebuilds — the bundle reads `document.documentElement` at import, so the renderer defers the require until the document exists). `_wireTooltips` auto-inits every `[data-bs-toggle="tooltip"]` (insert/retitle/dispose lifecycle via MutationObserver); full namespace at `window.bootstrap`/`manager.bootstrap`; docs/tooltips.md (incl. the disabled-control wrapper pattern).
- Tests: main fontawesome suite (aliases, traversal-proof sanitization, cache, IPC) + renderer suites for the REAL auto-render pipeline and tooltip lifecycle. Harness: renderer-preload now wires the Manager's DOM enhancements + mirrors `window.em.fontawesome` + exposes a tooltip probe (page-world synthetic events can't cross the contextIsolation split — single-world hover is covered by Somiibo's `test/boot/fontawesome-ui.test.js`). Docs indexed in CLAUDE.md; CHANGELOG entry added.
