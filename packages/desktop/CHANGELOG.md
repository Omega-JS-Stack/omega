...
### BREAKING
- **Config file is now `config/omega.json5` — `config/electron-manager.json` is no longer read.** One-time migration per app: move shared sections (`brand`, `sentry`, `analytics`, `payment`, `firebaseConfig`, `theme`) to the top level unchanged; move everything desktop-specific (`app`, `autoUpdate`, `startup`, `releases`, `downloads`, `remoteConfig`, `restartManager`, `electronBuilder`, `cdp`, `windows`, `fileAssociations`, `protocols`) under `targets.desktop`; rename the per-OS `targets.{mac,win,linux}` block to `targets.desktop.platforms.{mac,win,linux}`; then delete the old file. Full mapping table: `docs/config.md` in the Omega monorepo. `EM_BUILD_JSON.config` now carries the RESOLVED config (target section overlaid at the top level, e.g. `config.platforms.win`); the schema + validator live in the bundled `@omegajs/config`, also exposed as `require('electron-manager/config')` — the scaffolded workflow's snap/signing gates use it (re-run `npx mgr setup` to update your workflow). Secrets-shaped keys (`…Secret`, `…privateKey`) in config now hard-fail: move them to `.env`.
