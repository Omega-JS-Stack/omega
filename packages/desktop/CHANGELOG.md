...
### BREAKING
- **Renamed: `electron-manager` → `@omegajs/desktop`.** The package, both remaining bins (`omega-desktop` + the universal `mgr`; `em` and `electron-manager` dropped), all subpath entrypoints (`require('@omegajs/desktop/main')`, `/preload`, `/renderer`, `/build`, `/config`, `/lib/*`), and the consumer gulpfile path. Consumer projects: update the devDependency + requires and re-run `npx mgr setup` (the scaffolded defaults + workflows regenerate under the new identity).
- **SCSS entry is `omega-desktop`** — `@use 'omega-desktop' as * with (...)` (a scoped package name cannot be a bare sass load-path filename). The entry file is `dist/assets/css/omega-desktop.scss`.
- **Framework-internal IPC channels are prefixed `desktop:`** (were `em:`) — e.g. `desktop:storage:get`, `desktop:auth:sync-request`. The `window.em` renderer global and the `em:__test:*` harness channels are unchanged (parked internals, like the `EM_*` env vars).
- **Framework test prefix is `desktop:`** — `npx mgr test desktop:build/config` (`mgr:`/`framework:` remain universal). Audit IDs renamed `EM-xx` → `DSK-xx`.
- `EM_*` env vars (`EM_BUILD_MODE`, `EM_IS_PUBLISH`, `EM_TEST_BOOT_PROJECT`, …) are KEPT — cross-framework env-prefix harmonization is parked.
- **Config file is now `config/omega.json5` — `config/electron-manager.json` is no longer read.** One-time migration per app: move shared sections (`brand`, `sentry`, `analytics`, `payment`, `firebaseConfig`, `theme`) to the top level unchanged; move everything desktop-specific (`app`, `autoUpdate`, `startup`, `releases`, `downloads`, `remoteConfig`, `restartManager`, `electronBuilder`, `cdp`, `windows`, `fileAssociations`, `protocols`) under `targets.desktop`; rename the per-OS `targets.{mac,win,linux}` block to `targets.desktop.platforms.{mac,win,linux}`; then delete the old file. Full mapping table: `docs/config.md` in the Omega monorepo. `EM_BUILD_JSON.config` now carries the RESOLVED config (target section overlaid at the top level, e.g. `config.platforms.win`); the schema + validator live in the bundled `@omegajs/config`, also exposed as `require('@omegajs/desktop/config')` — the scaffolded workflow's snap/signing gates use it (re-run `npx mgr setup` to update your workflow). Secrets-shaped keys (`…Secret`, `…privateKey`) in config now hard-fail: move them to `.env`.

### Added
- Folded from legacy electron-manager v1.13.0 (same-lineage content already present was verified, the missing half copied): FontAwesome SVGs serve with `overflow="visible"` (FA Pro 7 glyphs may draw outside their viewBox), the tooltip observer reads `data-bs-original-title` (title-only hosts no longer infinite-loop the renderer), plus their regression tests and doc sections. v1.12.1's `files` fix (`scripts/` in the tarball) was already present.
...
- **Classy theme: configurable `$min-contrast-ratio`** — declared `!default` in `themes/classy/_config.scss` and forwarded into Bootstrap, so consumers with a light brand color (e.g. Somiibo blue `#1FB7F2`) can keep white button/badge text: `@use 'omega-desktop' as * with ($primary: #1FB7F2, $min-contrast-ratio: 2)`.
...
### Fixed
- Boot runner: fixture dep symlinks now create the link's PARENT dir — a scoped name (`node_modules/@omegajs/desktop`) needs `node_modules/@omegajs` to exist first; the old code silently failed and the boot layer died with "No gulpfile found".
- push-secrets discoverRepo tests use neutral fixture URLs (they pinned the framework's own repo slug, which broke — and would break again — on any rename).

### Removed
- `wm:prod` / `wm:local` scripts (web-manager swap helpers — workspace resolution owns this now); the `electron-manager` self-devDependency (the workspace link serves self-resolution); `repository`/`bugs`/`homepage` fields (monorepo home pending publish).
...
### Added
- **Main-process session persistence** (`lib/auth-persistence.js`) — sign-in now survives app restarts. Renderers always persisted (IndexedDB, browser contexts); main is Node, where Firebase defaults to in-memory — the bridge now boots auth on a PLUGGABLE storage strategy (`webManager.authPersistence` config: `safeStorage` default / `none` / custom via `register()`). The safeStorage strategy encrypts with the OS vault (macOS Keychain / Windows DPAPI / kwallet-gnome — the same os_crypt machinery Chromium uses for its cookie jar; browser IndexedDB/localStorage are plaintext LevelDB, so this is STRONGER than "keep it in a browser session") and stores only ciphertext in `{userData}/em-auth-session.json`. Restore is offline-capable and lands at boot (~400ms verified live). Storage only — cross-process distribution stays the IPC sync protocol. Corrupt file / failed decrypt / unavailable vault degrade to signed-out with a logged reason, never a boot crash.
- **Renderers run web-manager's FULL auth cycle** — `Manager.initialize()` now calls `auth().listen()` (persistent), so every renderer fetches the Firestore account, resolves the subscription, and **auto-populates the `data-wm-bind` bindings** — EM apps can now use UJM/BXM-style reactive HTML (`@show auth.user`, `@text auth.account.plan.id`, …) with zero JS.
- **Main knows the REAL plan** — each renderer settle pushes `{ resolved, roles }` to main (`em:auth:account-resolved`, uid-guarded against stale pushes; re-offered whenever main announces an auth state change, closing the boot-order race). New main APIs: `manager.webManager.getResolvedPlan()` → `{ plan, active, trialing, cancelling }` and `getResolvedRoles()`; `em:auth:plan-changed` broadcast on change; cache cleared on sign-out. This is BXM's split ported: browser contexts resolve, the authority caches. Fixes consumer plan gates that read a never-populated field and treated every install as the free plan.
- New `main/auth-persistence` suite (5 tests: strategy registry + custom registration, firebase adapter round-trip, REAL safeStorage encrypt/decrypt + garbage tolerance, uid-guarded account-resolved intake incl. broadcast-once and sign-out clear). Docs: web-manager-bridge.md (§ Session persistence, § Public API, renderer cycle).

---
## 1.11.1 — web-manager-bridge: firebase is bundled, not runtime-resolved

### Fixed
- **The main-process auth bridge silently ran in no-op mode in every dev app** — firebase was loaded via a `webpackIgnore`'d runtime `import()`, which resolves relative to the CONSUMER's `main.bundle.js`: that walk never reaches EM's `node_modules` when EM is symlinked (`mgr install dev`) and depends on npm hoisting when installed. `handleAuthToken` (the `openAuthFlow` receiving end) was "ignoring" tokens in dev. Firebase is now BUNDLED by webpack from EM's module context (same treatment as json5), deterministic across dev/live/packaged; the load-failure reason is logged instead of swallowed. Found live completing the first dev sign-in round trip.

---
## 1.11.0 — the sign-in round trip: getAuthUrl + openAuthFlow (dev loopback return)

### Added
- **`manager.openAuthFlow()`** (main; `lib/auth-flow.js`) — the EM-owned sign-in round trip. Always opens the user's REAL default browser on the `getAuthUrl()` chain (never an embedded window — sign-in rides the user's own browser session). Production returns via the OS custom scheme as before; dev/test — where the scheme is NOT OS-registered (protocol.js registers only in production; macOS can't runtime-register unlisted schemes at all) — return to a ONE-SHOT loopback HTTP listener instead (RFC 8252 §7.3: ephemeral 127.0.0.1 port, nonce-checked `state`, 5-min self-expiry, `Connection: close` + idle-socket teardown), which feeds the token into the SAME `auth/token` deep-link pipeline. `manager.authFlow.cancel()` tears down a pending listener. Requires web-manager ≥ 4.3.4 on the website (loopback `isValidRedirectUrl` allowance in dev). New `main/auth-flow` suite (7 tests — real listener, real HTTP returns). Docs: environment-detection.md § URL helpers, deep-link.md § built-ins.
- **`getAuthUrl(env, returnUrl)`** — optional second arg overrides the chain's final hop (used by auth-flow's dev loopback return; default stays `<brand.id>://auth/token`).
- **`manager.getAuthUrl()`** (url-helpers, all four Manager contexts) — the sign-in URL that round-trips an auth token back into the app: `<site>/signin?authReturnUrl=<site>/token?authReturnUrl=<brand.id>://auth/token` (site = `getWebsiteUrl()`, so dev/test stay on the local website). The UJM site logs the user in (already-signed-in users bounce straight through), its `/token` page mints a Firebase custom token via BEM, and the final redirect deep-links the token into the built-in `auth/token` route → `webManager.handleAuthToken()` → `signInWithCustomToken`. Use it for every Sign-in affordance an app exposes — never the bare `/signin` page, which strands the login in the browser. Throws without `brand.id`. Docs: environment-detection.md § URL helpers.

### Changed
- **web-manager dependency → `^4.3.4`** — picks up the loopback `isValidRedirectUrl` allowance that pairs with `openAuthFlow()`'s dev return channel (and the https dev API URL fix).
- **`auth/token` deep-link built-in reads the MODERN shape only: `?authToken=`** (was `?token=`, which no website flow ever sent). Legacy-app formats (`?token=`, `?payload=`) are deliberately ignored — legacy support lives in UJM, whose token page now sends custom-scheme redirects with BOTH `authToken` and the legacy `payload` (its wrap went additive; each consumer reads its own param).

---
## 1.10.1 — `mgr cdp --port` + macOS dock auto-rehide for hidden-mode apps

### Added
- **`mgr cdp --port <n>`** — every cdp subcommand now accepts a port flag that overrides `EM_CDP_PORT`. Needed because pinning the port in an npm script via `npx cross-env EM_CDP_PORT=…` mangles quoted eval expressions (cross-env strips inner quotes → `SyntaxError: missing ) after argument list`); `--port` keeps the expression intact. Documented in `docs/cdp-debugging.md` with the cross-env warning.
- **macOS dock auto-REHIDE for hidden-mode apps** — `startup.mode: 'hidden'` (LSUIElement) apps already got `app.dock.show()` when a window surfaced; now hiding the LAST visible named window (hide-on-close X, `windows.hide()`, any `win.hide()`) runs `app.dock.hide()` again, so the app returns to its fully-invisible posture instead of stranding a dock icon. Normal-mode apps unaffected; quit paths untouched. (`window-manager._maybeRehideDock`, wired to every named window's `hide` event.)

---
## 1.10.0 — Restart Manager protocol v2 · bundled FontAwesome + tooltips · electron-store bundled

### Fixed (storage)
- **Storage silently no-op'd in packaged consumer apps.** electron-store (ESM-only) was loaded via a `webpackIgnore`'d dynamic import — a runtime resolution that worked in dev (node_modules present) but failed in packaged builds, where EM is a devDependency that never ships in the asar; `manager.storage` fell back to the no-op and consumers had to install `electron-store` themselves to get persistence. `lib/storage.js` now uses a static-specifier `import(/* webpackMode: "eager" */ 'electron-store')`, so webpack inlines it into `main.bundle.js` — **consumers install nothing** (drop `electron-store` from consumer `dependencies` if you added it as a workaround). `utils/import-esm.js` (single-purpose, now orphaned) is deleted.
- New boot-layer regression suite (`boot/storage-bundled`): asserts no live `import('electron-store')` survives in the built consumer bundle (the packaged failure mode) + a real store round-trip with an on-disk `em-storage.json` through the bundle.

### Added (FontAwesome + Bootstrap tooltips)
- **FontAwesome ships in EM** (`docs/fontawesome.md`): the Font Awesome Pro icon set (solid + brands SVGs, incl. classic alias filenames) is bundled at `assets/icons/font-awesome/` and served by the new `lib/fontawesome.js` (`manager.fontawesome.get(name, style)` → svg | null; slug-sanitized, cached, `em:fontawesome:get` IPC). Renderers auto-render any `<i class="fa-solid fa-*">` element — static or dynamically inserted (MutationObserver) — injecting the SVG inline at `1em`/`currentColor`; `window.em.fontawesome.get()` exposes raw lookups. Zero consumer setup, fully offline.
- **Bootstrap JS ships in EM** (`docs/tooltips.md`): a prebuilt UMD bundle of the vendored Bootstrap 5.3 source with Popper inlined (`assets/themes/bootstrap/js/bootstrap.bundle.js`; `@popperjs/core` added as a dependency for rebuilds). The renderer bootstrap auto-initializes every `[data-bs-toggle="tooltip"]` — including elements inserted later; `data-bs-title` changes update the live instance and removals dispose it (no orphaned tips). The full namespace is exposed at `window.bootstrap` / `manager.bootstrap`; other components' standard data-api works on plain Bootstrap markup.
- Test coverage: main suite for fontawesome resolution/sanitization/IPC; renderer suites for the live auto-render pipeline and the tooltip lifecycle (auto-init, retitle, dispose). The renderer test-harness preload now wires the Manager's DOM enhancements and exposes `window.em.fontawesome` + a tooltip probe.

### Added (web-manager-bridge)
- **`manager.webManager.getIdToken()`** — fresh Firebase ID token for the signed-in user (async; null when signed out / Firebase absent), so consumer MAIN code can call authenticated backend routes (`Authorization: Bearer <token>`) without reaching into the bridge's private auth instance. Documented in `docs/web-manager-bridge.md`; unit-covered in the bridge main suite.

### BREAKING (Restart Manager protocol v2 — localhost HTTP, PID-exact, RM self-updates)
- **The legacy `restart-manager://message?command=register&payload=<json>` deep-link protocol is GONE.** `lib/restart-manager.js` (flat) is replaced by `lib/restart-manager/` — the registering side now talks to the rewritten Restart Manager app (restart-manager/restart-manager-desktop) over a loopback HTTP protocol advertised via `<appData>/restart-manager/runtime.json`, and RM monitors exact PIDs instead of process names. **Consumers need zero changes** (`restartManager: { enabled: true }` still validates); machines with only the LEGACY RM installed will treat RM as not-installed and install the new one alongside.

### Added (Restart Manager)
- **Protocol v1 SSOT** (`lib/restart-manager/protocol.js`, exported as `electron-manager/lib/restart-manager/protocol`): shared-root layout (runtime.json / app/ / install.lock), endpoint constants (`/v1/health|register|deregister|apps`), payload validators, `resolveSharedRoot` with the `EM_RM_ROOT` isolation seam, and `getInstalledAppPath` (mac/linux in the shared root's `app/`; win at the NSIS per-user location `%LOCALAPPDATA%\Programs\Restart Manager\`). The RM app imports this file — the contract can't drift between repos.
- **Full register lifecycle**: probe (runtime.json → pid-alive → health) → register with the app's OWN pid; 60s idempotent re-register heartbeat that also revives a dead RM; reliable deregister-on-quit (preventDefault-once + 1s-capped flush, auto-updater `quitAndInstall` path never intercepted).
- **Silent install everywhere** with smart existence (installed app on disk short-circuits before any network): mac zip → unzip (unchanged), **win NSIS one-click run with `/S` (fully silent, per-user, no admin — and the shape electron-updater can self-update)**, **linux AppImage + chmod (browser-.deb dropped)**. Release discovery via `releases/latest/download/latest*.yml` (default feed `restart-manager/update-server`, overridable via `restartManager.feed.{owner,repo}` or full `feed.url` for mirrors); atomic app.tmp swap on mac/linux; advisory install.lock (10-min stale takeover) against concurrent multi-app installs; 1h failure cooldown.
- **RM self-updates via EM's standard autoUpdater** — the lib only installs RM when missing; RM keeps itself fresh like any other EM app (registrations are storage-persisted on RM's side, so they survive its update relaunch; heartbeats reconnect via the fresh runtime.json). Zero bespoke update logic.
- New API: `ensureRunning()`, `getStatus()`; schema entries for `restartManager.{enabled,feed.owner,feed.repo,feed.url}`.
- Test coverage: build suites for the protocol SSOT + install machinery (feed parse/pick/URLs/lock), rewritten main suite with a real fixture server implementing the contract (register/deregister round-trips, heartbeat idempotence, dead-server resilience, quit-flush, testing-root isolation), boot suite proving the split lib bundles, extended-mode live-feed test (skips until releases publish).

### Removed (Restart Manager)
- `app.getApplicationNameForProtocol` probing, `_buildUrl`/`_send`, the hardcoded `restart-manager/download-server` URLs + `_urls` override seam, and the browser-.deb flow.

### Fixed (boot + docs)
- **A duplicate instance (lost single-instance lock) no longer runs the consumer's boot.** `Manager.initialize()` quit the duplicate but then RESOLVED, so the consumer's `.then()` ran its entire main.js (servers, IPC, shared files) against a dying Electron — observed live as a duplicate restart-manager clobbering the real instance's runtime.json with a dead-pid advertisement. initialize() now halts the boot (never-settling promise; the process exits first). Acute for agents like restart-manager around update relaunches.
- **docs/installer-options.md**: the `electronBuilder.mac.target` example implied targets MERGE — `deepMerge` REPLACES arrays, so the old example (`[{ target: 'mas' }]` "as an extra target") would silently drop dmg+zip and break auto-update. The example now restates the full list and documents the array-replace rule.

## 1.9.2 — `--extended` flag fix + consumer-template extended-mode docs
...
- **Consumer template now documents extended mode.** The 1.9.0 changelog claimed `src/defaults/CLAUDE.md` documented `--extended`/`TEST_EXTENDED_MODE`, but only the framework's root CLAUDE.md and the CI workflow carried it — scaffolded consumers never saw it. The template's test section now includes the `npx mgr test --extended` line (mirrors BXM/UJM wording).
- **Removed dead `EM_TEST_SKIP_INTEGRATION` from `.env.example`** — leftover from the removed pre-standardization `--integration` gate; nothing reads it.

## 1.9.1 — Test-harness focus + brand:// fixes, CDP doc rewrite

### Fixed
- **Test runs no longer steal keyboard focus via `webContents.focus()`.** The stealth recipe patched `win.show()`/`win.focus()`, but `webContents.focus()` is a different object that reaches the native window directly — it made the invisible (opacity-0) test window KEY, grabbing the keyboard from whatever the developer was typing in. The accessory activation policy doesn't help here: it prevents *launch* activation, not key-window steals. Consumers hit this through legitimate calls (e.g. focusing the address bar on tab select during boot suites). Now a global `web-contents-created` hook (main.js test-stealth block, same per-contents `EM_TEST_SHOW` predicate) no-ops `focus()` on every webContents during test runs — synthetic input (`executeJavaScript`, `sendInputEvent`, CDP) targets a specific webContents and never depended on OS key status. Verified by frontmost-app sampling: full consumer suite (492) + full framework suite (683) with zero focus transitions; all focus-sensitive boot suites (shortcuts, address-bar autofocus, omnibox) still green.
- **Main-layer test harness: consumer `brand://` loads no longer launch the installed production app.** Consumer main-layer suites that load `<brand.id>://` URLs (tab managers, internal pages) hit an unhandled scheme in the harness — the consumer's `main.js`, where the real protocol handler lives, never runs there. Chromium classified those loads as external protocols and handed them to the OS; on macOS, Launch Services opened the INSTALLED copy of the app that owns the scheme (e.g. `/Applications/Brand.app`) mid-test-run. The harness now (1) registers a stub in-process handler for the consumer's brand scheme on the default session, and (2) denies the `openExternal` permission on every session (partitions included) so no scheme can escape to the OS during tests. See `docs/test-framework.md` → "Test harness internals".

### Added
- **OMEGA mirror mandate** — `CLAUDE.md` (Doc-update parity) now states the docs are structurally MIRRORED across the sister frameworks (section skeleton, consumer template, shared-concept doc filenames, omega skills — same order everywhere); `src/defaults/CLAUDE.md` carries a maintainer mirror note (and its `## Per-process imports` header was renamed to the canonical `## Per-context imports`). Canonical skeletons live in the `omega:main` skill's mirror-spec resource.

### Changed
- **`docs/cdp-debugging.md`: Chrome section rewritten for the per-session isolated browser model** (mirrored across UJM/BEM/BXM/EM/WM). Driving a regular Chrome now = just call the `chrome-devtools` MCP tools (each session auto-launches its own private Chrome — no `open -gna` launch, no port 9223, no shared agent profile, no `CHROME_CDP_PORT`). **Everything Electron is unchanged**: `mgr cdp` + the `chrome-devtools-electron` upstream still attach to the running app by port (`EM_CDP_PORT`, default 9222). Fixed the stale upstream path (`servers/chrome-devtools-electron.json` → `servers/chrome-devtools-electron/config.json`) and the config snippet (absolute npx path).
