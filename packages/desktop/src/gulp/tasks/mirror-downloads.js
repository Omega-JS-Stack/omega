  ...
  Promise.resolve().then(async () => {
    ...
    try {
      ...
      const { data } = await octokit.rest.repos.createRelease({
        ...
        tag_name: tag,
        name: tag,
        body: `Latest installers (auto-mirrored by @omega.js/desktop). Stable filenames — direct links never change across versions.`,
        draft: false,
        prerelease: false,
      });
      releaseId = data.id;
...
// Naming convention preserves legacy URLs from before the v1 @omega.js/desktop rewrite — x64 builds
