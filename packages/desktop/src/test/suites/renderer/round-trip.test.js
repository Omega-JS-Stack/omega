// Renderer-layer round-trip tests. The window-omega-surface suite asserts the SHAPE
// of `window.desktop.*` (functions exist). This suite asserts the BEHAVIOR — that calls
...
module.exports = defineCases({
  ...
  tests: [
    {
      name: 'window.desktop.ipc.invoke: renderer → main → response',
      run: async (ctx) => {
        // Main registered the desktop:__test:echo channel — call it via the bridged ipc.
        const result = await window.desktop.ipc.invoke('desktop:__test:echo', { hello: 'from-renderer', n: 7 });
    ...
    {
      name: 'window.desktop.ipc.invoke: unhandled channel rejects',
      run: async (ctx) => {
        let threw;
        try {
          await window.desktop.ipc.invoke('desktop:__test:does-not-exist');
        } catch (e) { threw = e; }
    ...
    },
    {
      ...
      run: async (ctx) => {
        ...
        try {
          await window.desktop.storage.set(key, { changed: true });
    ...
    {
      name: 'window.desktop.autoUpdater.getStatus round-trips main state',
      run: async (ctx) => {
    ...
    {
      name: 'window.desktop.autoUpdater.onStatus subscribes (returns unsub fn)',
      run: (ctx) => {
        ...
        const off = window.desktop.autoUpdater.onStatus(() => {});
