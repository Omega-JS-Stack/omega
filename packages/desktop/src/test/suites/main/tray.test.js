...
//
// The harness boots in @omega.js/desktop's repo root, where there's no consumer `src/tray/index.js`,
...
module.exports = defineCases({
  ...
  cleanup: (ctx) => {
    ctx.omega.tray.destroy();
  },
  tests: [
    {
      name: 'initialize ran (enabled by default)',
      run: (ctx) => {
        ctx.expect(ctx.omega.tray._initialized).toBe(true);
      },
    },
    {
      ...
      run: (ctx) => {
        ...
        // declares what it wants; absent consumer → @omega.js/desktop ships its default template
        // (title, open, check-for-updates, quit, ...).
        const items = ctx.omega.tray.getItems();
    ...
    },
    {
      ...
      run: (ctx) => {
        ...
        ctx.expect(ctx.omega.tray.getTooltip()).toBe('TestApp');
      },
    },
    {
      name: 'define() throws on non-function input',
      run: (ctx) => {
        ctx.expect(() => ctx.omega.tray.define(null)).toThrow(/must be a function/);
        ctx.expect(() => ctx.omega.tray.define('nope')).toThrow(/must be a function/);
      },
    },
    {
      ...
      run: (ctx) => {
        ...
        ctx.expect(ctx.omega.tray.getItems().length).toBe(1);
    ...
    },
    {
      ...
      run: (ctx) => {
        ...
        ctx.omega.tray.addItem({ label: 'C' });
        const labels = ctx.omega.tray.getItems().map((i) => i.label);
    ...
    },
    {
      ...
      run: (ctx) => {
        ctx.omega.tray.addItem({ label: 'X' });
        ctx.omega.tray.clearItems();
        ctx.expect(ctx.omega.tray.getItems()).toEqual([]);
      },
    },
    {
      name: 'setIcon / setTooltip update state',
      run: (ctx) => {
        ctx.omega.tray.setIcon('/tmp/some-icon.png');
        ctx.omega.tray.setTooltip('Hover me');
        ctx.expect(ctx.omega.tray.getIcon()).toBe('/tmp/some-icon.png');
        ctx.expect(ctx.omega.tray.getTooltip()).toBe('Hover me');
      },
    },
    {
      ...
      run: (ctx) => {
        ...
        const resolved1 = ctx.omega.tray._resolveItem(ctx.omega.tray.getItems()[0]);
        ctx.expect(resolved1.label).toBe('Count: 0');

        count = 5;
        const resolved2 = ctx.omega.tray._resolveItem(ctx.omega.tray.getItems()[0]);
    ...
    },
    {
      ...
      run: (ctx) => {
        ...
        const resolved = ctx.omega.tray._resolveItem(ctx.omega.tray.getItems()[0]);
    ...
    },
    {
      ...
      run: (ctx) => {
        ...
        // We don't assert isRendered() because @omega.js/desktop auto-resolves an icon at init time, so
        ...
        ctx.omega.tray._icon = null;
        // Should not throw.
        ctx.omega.tray.refresh();
      },
    },
    {
      name: 'consumer function receives omega + tray builder (incl. id-path API)',
      run: (ctx) => {
        let receivedOmega;
        let receivedTray;
        ctx.omega.tray.define((arg) => {
          receivedOmega = arg.omega;
          receivedTray = arg.tray;
          arg.tray.item({ label: 'probe' });
        });
        ctx.expect(receivedOmega).toBe(ctx.omega);
    ...
    },
    {
      ...
      run: (ctx) => {
        ctx.omega.tray.define(({ tray }) => { tray.useDefaults(); });
        ctx.expect(ctx.omega.tray.find('open')).toBeTruthy();
        ctx.expect(ctx.omega.tray.find('quit')).toBeTruthy();
        ctx.expect(ctx.omega.tray.find('check-for-updates')).toBeTruthy();
      },
    },
    {
      ...
      run: (ctx) => {
        ctx.omega.tray.define(({ tray }) => { tray.useDefaults(); });
        const ok = ctx.omega.tray.update('quit', { label: 'GOODBYE' });
        ctx.expect(ok).toBe(true);
        ctx.expect(ctx.omega.tray.find('quit').label).toBe('GOODBYE');
      },
    },
    {
      ...
      run: (ctx) => {
        ctx.omega.tray.define(({ tray }) => { tray.useDefaults(); });
        ctx.expect(ctx.omega.tray.find('check-for-updates')).toBeTruthy();
        const ok = ctx.omega.tray.remove('check-for-updates');
        ctx.expect(ok).toBe(true);
        ctx.expect(ctx.omega.tray.find('check-for-updates')).toBe(null);
      },
    },
    {
      ...
      run: (ctx) => {
        ctx.omega.tray.define(({ tray }) => { tray.useDefaults(); });
        const ok = ctx.omega.tray.insertAfter('open', { id: 'probe', label: 'PROBE' });
        ctx.expect(ok).toBe(true);
        const items = ctx.omega.tray.getItems();
    ...
    },
    {
      ...
      run: (ctx) => {
        ctx.omega.tray.define(({ tray }) => { tray.useDefaults(); });
        ctx.omega.tray.hide('quit');
        ctx.expect(ctx.omega.tray.find('quit').visible).toBe(false);
        ctx.omega.tray.show('quit');
        ctx.expect(ctx.omega.tray.find('quit').visible).toBe(true);
        ctx.omega.tray.enable('quit', false);
        ctx.expect(ctx.omega.tray.find('quit').enabled).toBe(false);
        ctx.omega.tray.enable('quit');
        ctx.expect(ctx.omega.tray.find('quit').enabled).toBe(true);
      },
    },
    {
      ...
      run: (ctx) => {
        ctx.omega.tray.define(({ tray }) => { tray.useDefaults(); });
        ctx.expect(ctx.omega.tray.has('quit')).toBe(true);
        ctx.expect(ctx.omega.tray.has('does-not-exist')).toBe(false);
      },
    },
    {
      ...
      run: (ctx) => {
        ctx.omega.tray.define(({ tray }) => { tray.useDefaults(); });
        const ok = ctx.omega.tray.insertBefore('quit', { id: 'before-quit', label: 'BEFORE' });
        ctx.expect(ok).toBe(true);
        const items = ctx.omega.tray.getItems();
    ...
    },
    {
      ...
      run: (ctx) => {
        ...
        const ok = ctx.omega.tray.appendTo('parent', { id: 'child', label: 'Child' });
        ctx.expect(ok).toBe(true);
        const parent = ctx.omega.tray.find('parent');
        ctx.expect(Array.isArray(parent.submenu)).toBe(true);
        ctx.expect(parent.submenu[0].id).toBe('child');
        // Resolves a nested id-path too:
        ctx.expect(ctx.omega.tray.find('parent/child').label).toBe('Child');
      },
    },
    {
      ...
      run: (ctx) => {
        ...
        ctx.expect(ctx.omega.tray.find('account/sign-out').label).toBe('Sign out');
        ctx.omega.tray.update('account/sign-out', { label: 'Bye' });
        ctx.expect(ctx.omega.tray.find('account/sign-out').label).toBe('Bye');
      },
    },
    {
      ...
      run: (ctx) => {
        ...
        ctx.omega.autoUpdater._updateTrayItem();

        const item = ctx.omega.tray.find('check-for-updates');
