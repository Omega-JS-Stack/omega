  ...
  tests: [
    ...
    // and the precheck would be handed a .p8 no filesystem can answer for.
    {
      ...
      run: async (ctx) => {
        ...
        // file and `security import` of one dies with "parameters not valid".
        // A MISSING one exits 1 (#891): a mac build this framework ships is signed
        // and notarized, and the message names the command that fixes it.
    ...
    // The trigger half of the signer guard
    // ([#875](https://github.com/Omega-JS-Stack/omega/issues/875)): the box that
    // holds the EV token is reachable by a DISPATCH and nothing else, so a
    // `push` or `pull_request` trigger added to this file later has to SKIP the
    // sign job rather than sign a stray build with it.
    {
      name: 'the sign job runs on a dispatch only, and a skipped sign job still reaches finalize (#875)',
      run: async (ctx) => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'desktop-workflow-signer-guard-'));
        await copyDefaults(tmp);
        const { jobs } = yaml.load(jetpack.read(path.join(tmp, '.github', 'workflows', 'build.yml')));

        const gate = jobs['windows-sign'].if;
        // The ONE trusted event (#923), ANDed onto the rest so the strategy and
        // platform gates still hold on a dispatch.
        ctx.expect(gate).toContain("&& github.event_name == 'workflow_dispatch'");
        ctx.expect(gate).not.toContain('repository_dispatch');
        ctx.expect(gate).toContain("needs.windows-strategy.outputs.strategy != 'local'");

        // A SKIP, never a failure: finalize reaches its `if` through `always()`
        // and gates on the BUILD's result, so the unsigned Windows artifact the
        // build job uploaded still finishes the run.
        ctx.expect(jobs.finalize.if).toContain('always()');
        ctx.expect(jobs.finalize.if).toContain("needs.build.result == 'success'");
        ctx.expect(jobs.finalize.if).not.toContain('needs.windows-sign.result');
    ...
    // header, the same one dispatch trigger, a version log, and a bound on
    // every job. Read off the scaffolded render, like every case above.
    {
      name: 'the scaffolded workflow carries the sibling header, the one dispatch trigger, a version log and a timeout per job (#880)',
      run: async (ctx) => {
        ...
        // The one dispatch trigger (#923), carrying the `platforms` input.
        // js-yaml 4 keeps `on` a plain string key (YAML 1.2 drops the
        // `on`/`off` booleans YAML 1.1 had), so `doc.on` is the trigger map.
        ctx.expect(Object.keys(doc.on)).toEqual(['workflow_dispatch']);
        ...
        for (const command of ['node -v', 'npm -v', 'node "${{ github.workspace }}/node_modules/@omega.js/desktop/bin/omega" version']) {
