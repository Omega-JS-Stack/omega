...
// A desktop target inside a brand monorepo whose brand names a COMPANY (#677):
// the parent brand's company/ tree holds the shared .env, and the machine
// registry line (the parent's own runs write it) is what says where it is.
function seedBrand() {
  ...
  jetpack.write(path.join(brandRoot, 'config', 'omega.json5'), '{ brand: { id: "fixture" }, company: { id: "fixture-co" }, targets: { desktop: { type: "desktop" } } }');
