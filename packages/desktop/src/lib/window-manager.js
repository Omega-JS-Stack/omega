...
// time `create()` or `show()` runs, @omega.js/desktop calls `app.dock.show()` so the icon
...
const windowManager = {
  ...
  initialize(manager) {
    ...
    // opens the main window), surface `main`. We only show — never create — so @omega.js/desktop's "no
  ...
  async createNamed(name, manager, overrides) {
    ...
    // LSUIElement apps (startup.mode='hidden'): hiding the last visible window —
    // hide-on-close X, windows.hide(), any consumer hide() — drops the dock icon
    // again so the app returns to its fully-invisible posture. Inverse of the
    // dock.show() that surfacing performed. See _maybeRehideDock.
    win.on('hide', () => windowManager._maybeRehideDock());
  ...
  // Inverse of _ensureDockVisible, for LSUIElement apps only (startup.mode='hidden'):
  // when the LAST visible window hides, drop the dock icon (and with it the Cmd+Tab
  // entry) so the app disappears completely again — dock and UI appear and vanish
  // TOGETHER. Normal-mode apps keep their dock icon for life; quit paths are left
  ...
  _maybeRehideDock() {
    ...
    logger.log('_maybeRehideDock — last visible window hidden; dock.hide() (startup.mode=hidden)');
    dock.hide();
  },
