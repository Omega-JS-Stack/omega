...
- **Ad units with zero JS.** Drop `<div data-omega-ad></div>` into any view — the renderer auto-binds it (live, MutationObserver) to the shared OMEGA ads module: house/company inventory only (no AdSense in desktop surfaces), lazy near-viewport loading, sandboxed iframe, no-fill collapse. See [ads](docs/ads.md).
