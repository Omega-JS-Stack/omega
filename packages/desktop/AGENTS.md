...
- **`omega:em`** — router skill. Auto-loads on desktop-specific keywords (`manager.windows`, `manager.tray`, `electron-builder`, `npx omega setup`, etc.) and points back to this AGENTS.md + `docs/` (the SSOT), carrying only Claude-workflow hard rules and process checklists.
