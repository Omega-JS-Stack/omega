 ...
 * account is tri-state config now (#33) — `cloud.billingAccount` set →
...
module.exports = async function ensureBilling(context) {
  ...
  const billingAccountName = brandConfig.cloud?.billingAccount
  ...
  if (!billingAccountName) {
    console.log(`      ${chalk.dim('→')} Set cloud.billingAccount ("billingAccounts/XXXXXX-XXXXXX-XXXXXX") in company/brand config to auto-upgrade`);
    ...
    return needsInteractiveSkip(
      ...
      'no cloud.billingAccount configured',
    );
  }
