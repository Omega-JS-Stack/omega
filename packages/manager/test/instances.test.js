...
test('testing: each web instance is checked on ITS url; a live record-less instance adopts under its own key', async () => {
  ...
  assert.equal(state.deploy['web:admin'].adopted, true, 'live hit adopted under the instance key');
  assert.equal(state.deploy.web.adopted, undefined, 'the primary record was not rewritten');
});
