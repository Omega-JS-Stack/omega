...
test('runManage: full loop — workspace, update (build), testing all pass; run output + gitignore written', async () => {
  ...
  assert.deepEqual(run.services.map((s) => s.service), ['workspace', 'github', 'cloudflare', 'domain', 'cloud', 'recaptcha', 'analytics', 'search-console', 'adsense', 'monitoring', 'campaigns', 'newsletter', 'payment', 'slapform', 'chatsy', 'replyify', 'server', 'directory', 'assets', 'certificates', 'disperse', 'seo', 'update', 'account', 'migrations', 'bookmark', 'testing']);
