/**
 * GitHub service tests — the org/repo/pages reconciliation against a
...
const service = require('../src/services/github/index.js');
...
function run(config, api, options = {}) {
  ...
  return service.run({
    ...
    brandConfig: config,
    brand: { id: config.brand?.id, config, targets, apps: [] },
    brandState: {},
    apps: [],
...
test('github: github.enabled = false skips the service', async () => {
  const result = await run(brandConfig({ enabled: false }), fakeApi());
  assert.equal(result.status, 'skipped');
});

test('github: shared org skips org reconciliation but still reconciles the repo', async () => {
...
test('github: fully converged brand is a zero-mutation no-op with state intact', async () => {
