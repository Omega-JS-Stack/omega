...
// ([#168](https://github.com/Omega-JS-Stack/omega/issues/168)): a web app whose
...
/**
 * Stage a brand monorepo with one website app. `translation` is the raw
 * section for the BRAND config (the shared layer every app inherits); `pkg`
 * overrides the app's package.json.
 */
function stageBrand({ translation, pkg, target = 'web' } = {}) {
  ...
  const appName = target === 'web' ? 'website' : target;
  const appPath = path.join(root, 'apps', appName);
  fs.mkdirSync(appPath, { recursive: true });
  fs.writeFileSync(path.join(appPath, 'package.json'), `${JSON.stringify(pkg || {
    name: `fixture-${appName}`,
    private: true,
    dependencies: { '@omega.js/web': '^0.1.0' },
  }, null, 2)}\n`);

  return { root, appPath, apps: [{ name: appName, dir: `apps/${appName}`, path: appPath, target }] };
}
...
const readPkg = (appPath) => JSON.parse(fs.readFileSync(path.join(appPath, 'package.json'), 'utf8'));
...
test('translation-sdk: claude translation + no dep → package.json gains it and npm install runs', async () => {
  const { root, appPath, apps } = stageBrand({ translation: CLAUDE });
  const run = fakeRun();

  const result = await translationSdkOp({ brandRoot: root, apps, runCommand: run });
...
test('translation-sdk: the range comes from the INSTALLED web package\'s optional peer', async () => {
  ...
  await translationSdkOp({ brandRoot: root, apps, runCommand: fakeRun() });

  assert.equal(readPkg(appPath).dependencies[SDK], '>=9.9');
});
...
test('translation-sdk: explicit enabled + provider claude drifts the same way', async () => {
  const { root, appPath, apps } = stageBrand({
    translation: { enabled: true, provider: 'claude', languages: ['fr'] },
  });

  const result = await translationSdkOp({ brandRoot: root, apps, runCommand: fakeRun() });

  assert.deepEqual(result.output.translationSdk.added, ['website']);
  assert.ok(readPkg(appPath).dependencies[SDK]);
});
...
test('translation-sdk: dep already declared → null return, byte-identical package.json, no install', async () => {
  ...
  const before = fs.readFileSync(path.join(appPath, 'package.json'), 'utf8');
  const run = fakeRun();

  assert.equal(await translationSdkOp({ brandRoot: root, apps, runCommand: run }), null);
  assert.equal(fs.readFileSync(path.join(appPath, 'package.json'), 'utf8'), before);
  assert.deepEqual(run.calls, []);
});

test('translation-sdk: a devDependency declaration counts as converged', async () => {
  const { root, apps } = stageBrand({
    translation: CLAUDE,
...
test('translation-sdk: a second pass over a healed app changes nothing', async () => {
  const { root, appPath, apps } = stageBrand({ translation: CLAUDE });

  await translationSdkOp({ brandRoot: root, apps, runCommand: fakeRun() });
  const healed = fs.readFileSync(path.join(appPath, 'package.json'), 'utf8');

  const run = fakeRun();
  assert.equal(await translationSdkOp({ brandRoot: root, apps, runCommand: run }), null);
  assert.equal(fs.readFileSync(path.join(appPath, 'package.json'), 'utf8'), healed);
  assert.deepEqual(run.calls, []);
});
...
test('translation-sdk: dry run plans the add + install and writes nothing', async () => {
  ...
  const result = await translationSdkOp({ brandRoot: root, apps, options: { dryRun: true }, runCommand: run });
...
test('translation-sdk: translation off (no languages, or enabled false) is untouched', async () => {
  for (const translation of [undefined, { languages: [] }, { enabled: false, languages: ['es'] }]) {
    const { root, appPath, apps } = stageBrand({ translation });
    const before = fs.readFileSync(path.join(appPath, 'package.json'), 'utf8');
    const run = fakeRun();
...
test('translation-sdk: the chatgpt provider needs no SDK, so the app is untouched', async () => {
  const { root, appPath, apps } = stageBrand({
...
test('translation-sdk: an unreadable translation section warns instead of halting the walk', async () => {
  const { root, appPath, apps } = stageBrand({ translation: { languages: ['zz'] } });
  const before = fs.readFileSync(path.join(appPath, 'package.json'), 'utf8');
  const run = fakeRun();
  ...
  assert.equal(fs.readFileSync(path.join(appPath, 'package.json'), 'utf8'), before, 'the app is left alone');
  assert.deepEqual(run.calls, []);
});

test('translation-sdk: non-web apps are untouched (their framework ships the SDK)', async () => {
  const { root, appPath, apps } = stageBrand({ translation: CLAUDE, target: 'extension' });
  const before = fs.readFileSync(path.join(appPath, 'package.json'), 'utf8');
  const run = fakeRun();
