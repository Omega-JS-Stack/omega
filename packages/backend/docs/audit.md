...
1. **Detect scope** — read `package.json` (consumer: `functions/package.json`): `name` is `@omega.js/backend` → **framework audit** (U + @omega.js/backend + F checks); `@omega.js/backend` in (dev)dependencies → **consumer audit** (U + @omega.js/backend checks).
...
Only when auditing the @omega.js/backend repo itself. Mirrored across the four frameworks.
