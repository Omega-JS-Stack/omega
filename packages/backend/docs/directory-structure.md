...
Consumers are src-first: authored code lives in `src/`, `functions/` is staged output (gitignored, generated by `omega build`). See [build-system.md](build-system.md).

```
package.json                          # THE app manifest: scripts + runtime deps
.env                                  # Secrets (gitignored; D15 cascade)
.nvmrc                                # Node version pin
service-account.json                  # Firebase SA (gitignored)
...
functions/                            # GENERATED — never edit (staged by omega build)
```
