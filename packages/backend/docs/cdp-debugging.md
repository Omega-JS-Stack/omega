...
> Mirrored across the five sister packages (web, backend, extension, desktop, client): the same core section, framework-flavored. Edit all five together.
...
An Electron dev app is a running singleton, so you ATTACH to it instead of launching a browser: the `chrome-devtools-electron` MCP upstream (reads `OMEGA_CDP_PORT`, default 9222, expanded once at session start) or the desktop framework's per-invocation `npx omega cdp`. See @omega.js/desktop's `docs/cdp-debugging.md`.

## @omega.js/backend specifics

- **The web dev site URL is `https://localhost:4000`, NEVER the LAN IP** (`https://192.168.x.x:...`). Port 4000 by convention, auto-bumped +1 when taken; `--port=N` or the config's `ports.website` pins it (see [docs/web/index.md](../../../docs/web/index.md)).
...
- Backend-side observation stays where it always was: `npx omega logs` (gcloud logs) and the emulator suite; this doc only covers the browser half of the loop.
