...
@omega.js/backend now mirrors every other OMEGA framework: **consumer backends are src-first**. Authored code lives in `src/`, and `functions/` is staged output — generated by `omega build`, gitignored wholesale, disposable.

## Pipeline overview

| Stage | Command | What happens |
|---|---|---|
| Build | `npx omega build` | Stage `src/` → `functions/` (1:1 copy, derived manifest, composed brand⊕app config, .env/.nvmrc/SA) |
...
apps/<brand>/apps/backend/
  ...
  functions/                # ← GENERATED (gitignored) — never edit
```

### Staged functions/ output

- `src/**` → `functions/**` (1:1 copy, no transforms — CJS as-is)
...
- `.env` / `.nvmrc` / `service-account.json` → verbatim copy
