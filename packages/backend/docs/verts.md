...
| `GET /omega/verts/redirect?id&parent` | public | fail-closed click hop: 302 ONLY to the ad's STORED http(s) link + `utm_source=<parent>`/`utm_medium=omega-vert`/`utm_campaign=<vertId>`; a utm key the advertiser already put on the link WINS (the tagger only fills what is missing, so nothing double-appends); unknown id or non-http link → 404; caller-supplied URLs are never redirected to. The tagging itself is `applyVertUtm` in `@omega.js/client`'s `modules/vert-document.js` — the same function the client's promo lane tags its own link with |
