...
@omegajs/backend syncs user data to marketing providers (SendGrid, Beehiiv) as custom fields. Field definitions live in a single dictionary; OMEGA provisions them in each provider.
...
4. @omegajs/backend resolves field IDs at runtime — no provider code changes needed.
...
- **Beehiiv**: @omegajs/backend uses the key directly as the custom field name — no ID resolution needed.
...
| Purpose | File |
|---------|------|
| Field dictionary (@omegajs/backend SSOT) | `src/manager/libraries/email/constants.js` |
