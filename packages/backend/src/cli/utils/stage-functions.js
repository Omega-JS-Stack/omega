 ...
 *                           brand⊕local flattened, each layer overlaid by its
 *                           own omega.<environment>.json5 when the STAGE names
 *                           an environment (#856); the deployed runtime cannot
...
const { resolveServiceAccountPath, SECRETS_DIR } = require('@omega.js/devkit/service-account');
 ...
 *   which `.env.<environment>` overlay composes into dist/.env (#586) AND which
 *   `omega.<environment>.json5` overlay composes into dist/config/omega.json5
 *   ([#856](https://github.com/Omega-JS-Stack/omega/issues/856)), one word for
 *   both halves of the upload. Each lane pins its own: a deploy 'production', a
 *   test lane 'testing', the local lanes (emulator, serve, mcp) 'development';
 *   omitted (a bare `omega build`) defaults to the running environment for .env
 *   and composes no config overlay.
...
function stageFunctions(options) {
  ...
  //     is not a config location (@omega.js/config CONFIG_LOCATIONS). The
  //     STAGE names the environment here too (#856), so the config overlay
  //     and the .env overlay below it are the same word. ───────────────────
  const { config } = composeTargetConfig(projectDir, 'backend', { environment: options.environment });
  ...
  const manifest = {
    ...
    private: true,
    // The target's own license rides into the artifact (#884): the uploaded
    // package states what the target states, and ensure-target seeds
    // UNLICENSED when a manifest states none.
    license: targetPackage.license || 'UNLICENSED',
    main: 'index.js',
...
function watchAndStage(options) {
  ...
  if (brandRoot) {
    ...
    watch(path.join(brandRoot, SECRETS_DIR), restage('brand secrets'));
  }
