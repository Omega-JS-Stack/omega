/**
 * Context request parsing — tolerant JSON5 body/query parsing, request-URL
...
function tryUrl(self, req) {
  try {
    const omega = self.omega;
    const projectType = omega.options.projectType;
    const protocol = req.protocol;
    const host = req.get('host');
    const forwardedHost = req.get('x-forwarded-host');
    const reqPath = req.path;
    const functionsUrl = omega.getFunctionsUrl();
...
const methods = {
   ...
   */
  parseMultipart(options) {
    const self = this;
    return new Promise(function (resolve, reject) {
      const existingData = self.request.multipart;
      ...
      bb.on('finish', async () => {
        ...
        return resolve(self.request.multipart);
      });
