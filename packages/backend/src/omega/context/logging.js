/**
 * Context logging — tagged, level-aware logging for one request.
 *
  ...
  _log() {
    ...
    if (LOG_LEVELS[level]) {
      ...
      if (level in console) {
        ...
        self.omega.firebase.functions.logger.write({
          severity: LOG_LEVELS[level].toUpperCase(),
          message: logs,
        });
      }

      // Write with wonderful-log (the logSavePath option)
      const fileLogger = self.omega.fileLogger;

      if (fileLogger && fileLogger[level]) {
        fileLogger[level](...logs);
      }
    } else {
      console.log.apply(console, logs);
    }
