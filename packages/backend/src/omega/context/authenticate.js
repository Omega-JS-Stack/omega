/**
 * Context authentication: resolves the calling user from the request into
 * `ctx.user`, a `User` from @omega.js/account.
 *
 ...
 *
 * The default call settles ONCE per request: the pipeline authenticates, and a
 * later `ctx.usage` resolve reads that same answer instead of verifying the
 * token and re-reading the user doc a second time.
 */

const safeCompare = require('../helpers/safe-compare.js');
const redactSecret = require('../helpers/redact-secret.js');
...
const methods = {
  authenticate(options) {
    ...
    // An explicit call (an in-process adminKey, resolve: false) always runs
    if (options) {
      return resolveCaller(self, options);
    }

    self._services.authentication = self._services.authentication || resolveCaller(self, {});
 ...
 * @param {object} options - { resolve, debug, adminKey, authenticationToken, apiKey }
 * @returns {Promise<User|object>} ctx.user when resolving, else the raw caller.
 */
async function resolveCaller(self, options) {
  // Shortcuts
  const admin = self.omega.firebase.admin;
  const req = self.req;
  const data = self.request ? self.request.data : {};

  // The caller as the lanes below build it: the document, plus the
  // authenticated verdict and the sign-in's identity
  let user = { ...self.user.toJSON(), authenticated: self.user.authenticated };
  let identity = null;
  ...
  function _resolve(user) {
    ...
    // Resolve the user
    if (options.resolve) {
      const resolved = new User(user, identity);
      const authenticated = user.authenticated || false;

      // The verdict the lanes reached wins over the getter's uid rule: the
      // admin key authenticates a request that carries no account at all
      if (resolved.authenticated !== authenticated) {
        Object.defineProperty(resolved, 'authenticated', { value: authenticated, writable: true });
      }

      self._services.user = resolved;
  ...
  if (isJWT(idToken)) {
    ...
    try {
      ...
      if (isUserDoc(userDoc) || existed) {
        user = Object.assign({}, user, userDoc);
        user.authenticated = true;
        user.auth.uid = decodedIdToken.user_id;
        user.auth.email = decodedIdToken.email;

        // The profile rides the token's claims (the API key lane has none)
        identity = {
          uid: decodedIdToken.user_id,
    ...
    await admin.firestore().collection(`users`)
      ...
      .then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
          user = Object.assign({}, user, doc.data());
          user.authenticated = true;
        });
      })
      .catch((error) => {
