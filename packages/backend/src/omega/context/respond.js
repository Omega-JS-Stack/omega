/**
 * Context response door — the ONE home of the wire contract.
 *
...
const { redactResponseForLog } = require('../helpers/log-redaction.js');
...
const methods = {
  ...
  report(e, options) {
    ...
    if (isBetween(options.code, 500, 599)) {
      ...
      self.omega.sentry?.captureException?.(newError);
    } else {
...
function attachHeaderProperties(self, options, error) {
  ...
  const headers = {
    ...
    // stays synchronous.
    usage: {
      current: self.usage.counters(),
      limits: self.usage.limits(),
    },
    // No resolved schema map rides the header: nothing has ever set one
    schema: {},
