...
/**
 * Utilities: the process service of shared helpers (`omega.utilities`), built
 * once at boot: Firestore and Auth iteration, the owner-user document read,
 * the cached document read, id minting, and the slug, sanitize and trim passes
 * the request pipeline and the routes share.
 */
class Utilities {
  /**
   * @param {object} omega - the Omega instance (Firestore and Auth through `omega.firebase.admin`).
   */
  constructor(omega) {
    // Libraries
    _ = require('lodash');
  ...
  iterateCollection(callback, options) {
    const self = this;
  ...
  getDocumentWithOwnerUser(path, options) {
    const self = this;
    ...
    return new Promise(async function(resolve, reject) {
      ...
      const userResolved = new User(user);
      ...
      if (options.resolve.schema) {
        const documentResolved = new Settings(options.resolve.ctx).resolve({
