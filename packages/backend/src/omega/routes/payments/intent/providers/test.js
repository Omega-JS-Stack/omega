...
async function createSubscriptionIntent({ uid, orderId, product, frequency, trial, discount, declined, abandoned, confirmationUrl, ctx }) {
  ...
  // This lets the test provider work in brands that haven't wired up Stripe yet.
...
async function waitForOrder({ orderId, ctx }) {
  const ref = ctx.omega.firebase.admin.firestore().doc(`payments-orders/${orderId}`);
  ...
  ctx.warn(`Test provider: payments-orders/${orderId} never landed after ${(ORDER_WAIT_ATTEMPTS * ORDER_WAIT_INTERVAL) / 1000}s — firing the failed invoice anyway, it will resolve against whatever state exists`);
  return false;
}
...
function fireWebhook({ event, ctx }) {
  const webhookUrl = `${ctx.omega.getApiUrl()}/omega/payments/webhook?provider=test&key=${env.get('OMEGA_WEBHOOK_KEY')}`;
  return fetch(webhookUrl, {
    ...
    ctx.log(`Test provider auto-webhook failed: ${e.message}`);
  });
}
