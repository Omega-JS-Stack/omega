...
module.exports = async function dispatchDeploy(ctx, octokit, data) {
  data.deploy = data.deploy !== false;
  if (!data.deploy) {
    return data;
  }
  ...
  const workflow = composedWorkflowNameFor(data.target, WORKFLOW);
