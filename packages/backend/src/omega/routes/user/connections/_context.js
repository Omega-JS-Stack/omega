 ...
 *
 * @param {object} data - The resolved route input
 * @returns {object|null} The error, or null when no uid was passed
 */
function droppedUidError(data) {
  if (!data.uid) {
    return null;
  }
 ...
 */
function returnUrlError(data) {
  if (!data.returnUrl) {
    return null;
  }

  if (isSitePath(data.returnUrl)) {
    return null;
  }
 ...
 */
async function buildContext({ ctx, user, data, requireProvider = true, honorUid = false }) {
  ...
  const targetUid = (honorUid && data.uid) || user.auth.uid;
  ...
  const resolved = loadProvider(data.provider, omega);
