...
// `${omega.cwd}/<dir>/<name>.js` convention `src/routes/` and the event
 ...
 *
 * @param {object|null} omega - The Omega instance (its `cwd` is the staged consumer dir)
 * @returns {string[]} Absolute directory paths, brand first
 */
function providerDirs(omega) {
  const dirs = [];

  if (omega && omega.cwd) {
    dirs.push(path.join(omega.cwd, BRAND_PROVIDERS_DIR));
  }
 ...
 * @param {object|null} omega - The Omega instance
 ...
 */
function resolveProvider(providerName, omega) {
  for (const dir of providerDirs(omega)) {
    let file;
 ...
 */
function loadProvider(providerName, omega) {
  const connectionProvider = resolveProvider(providerName, omega);
