 ...
 *
 * @param {object} ctx - The Context the metadata stamp logs through
 ...
 */
function buildUserDoc({ ctx, user, tag }) {
  const providerName = extractProviderName(user);

  // The full account shape (.js/account User)
  ...
  const meta = ctx.metadata({ tag: tag });
 ...
 *
 * @param {object} ctx - The Context to log through
 ...
 */
async function healUserDoc({ ctx, admin, uid }) {
  ...
  const record = buildUserDoc({ ctx, user: authUser, tag: HEAL_TAG });
