 ...
 *
 * Ghostii (api.ghostii.ai) is UNOPINIONATED about @omega.js/backend. It returns a generic,
 ...
 * @omega.js/backend's post format — @omega.js/backend is responsible for transforming Ghostii's output into
 ...
 *   blocksToPost() consumes to build @omega.js/backend's post shape.
 */
async function writeArticle({ brand, description, links, sourceContent, overrides }) {
  ...
  const body = {
    ...
    // backend whose auth reads exactly this payload field. Renames to the
...
/**
 * Transform Ghostii's generic JSON block array into @omega.js/backend's post shape.
 *
 * Ghostii returns `json: [{ name, content }]` where name ∈ heading-1..6, image,
 * paragraph, blockquote, list. @omega.js/backend's admin/post wants the title + header image as
...
async function publishArticle(ctx, { brand, article, id, author, postPath, source }) {
  const apiUrl = ctx.omega.getApiUrl();
  ...
  // The post template sets no permalink, so the public URL follows the
