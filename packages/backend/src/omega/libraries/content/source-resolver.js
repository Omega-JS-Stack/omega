 ...
 * @param {object} [args.omega] - Omega instance (parent URL resolution)
 * @param {object} args.ctx - logger
 * @returns {object} state
 */
function createResolverState({ sources, categories, admin, omega, ctx }) {
  return {
    pool: [...(sources || [])],
    categories: categories || [],
    admin,
...
async function loadParentPool(state) {
  ...
  const parentUrl = state.omega?.getParentApiUrl?.();
  if (!parentUrl) {
  ...
  for (const category of categoriesToFetch) {
    const query = {
      ...
      // omega-admin-key header (sent below); older parents read exactly
 ...
 */
async function resolveSources({ sources, count, categories, admin, omega, ctx }) {
  count = count || 1;

  const state = createResolverState({ sources, categories, admin, omega, ctx });
