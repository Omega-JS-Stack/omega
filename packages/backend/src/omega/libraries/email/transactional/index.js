...
function Transactional(ctx) {
  ...
  self.admin = self.omega.firebase.admin;
...
Transactional.prototype.build = async function (settings) {
  ...
  // --- 1. Brand + sender ---
  const { brand, brandDomain, company } = prepare.resolveBrand(omega);
  const { from, groupId } = prepare.resolveSender(settings, brand, brandDomain, omega);
  ...
  const userProperties = new User(rawUserDoc).toJSON();
  ...
  const unsubscribeUrl = prepare.buildUnsubscribeUrl({
    ...
    websiteUrl: omega.project.websiteUrl,
  });
...
Transactional.prototype.send = async function (settings) {
  ...
  if (capture.isCapturing(ctx)) {
    const record = capture.recordCaptured(omega, {
      to: email.to,
  ...
  // Initialize SendGrid
  const sendgrid = omega.require('@sendgrid/mail');
