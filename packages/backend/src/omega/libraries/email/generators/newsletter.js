 ...
 *
 * Pipeline:
 *   1. Read newsletter categories from omega.config.marketing.newsletter.content.categories
 ...
 *
 * @param {object} omega - the @omega.js/backend instance
 ...
 */
async function generate(omega, ctx, settings, opts = {}) {
  ...
  const newsletterRoleConfig = omega.config?.marketing?.newsletter;
  ...
  const brand = omega.config?.brand;
  const ai = ctx.ai;
  ...
  if (USE_CDN_URLS && host === 'github') {
    const companyUrl = omega.config?.company?.url || brand?.url;
  ...
  await sendNewsletterReportEmail(omega, ctx, {
    brand,
    subject: structure.subject,
    preheader: structure.preheader,
 ...
 */
async function buildLinkedArticle({ omega, ctx, brand, config, structure, sources, publish }) {
  const lead = structure.sections[0] || {};
  const publicConfig = buildPublicConfig(omega.config);
  ...
  const slug = omega.utilities.slugify(article.title || '');
  const url = slug
 ...
 */
async function sendNewsletterReportEmail(omega, ctx, args) {
  const brandDomain = omega.config?.brand?.contact?.email?.split('@')[1];
  ...
  try {
    ...
    // The report body is hand-built markup (trustedContent below). Everything it
    // interpolates is AI-authored (subject, preheader, tags) or scraped from a source
    // (titles, URLs), so it gets escaped on the way in.
    const messageLines = [];
    ...
    if (beehiivOk) {
      ...
      messageLines.push(`<li>${escapeHtml(args.beehiivFailureReason || 'unknown')}</li>`);
      messageLines.push('</ul>');
    }
    messageLines.push('');
    ...
    messageLines.push('<ul>');
    messageLines.push(`<li>Subject: ${escapeHtml(args.subject)}</li>`);
    messageLines.push(`<li>Preheader: ${escapeHtml(args.preheader || '(none)')}</li>`);
    if (args.tags?.length) {
      messageLines.push(`<li>Tags: ${escapeHtml(args.tags.join(', '))}</li>`);
    }
    messageLines.push('</ul>');
    messageLines.push('');
    ...
    if (args.sources?.length) {
      ...
      for (const source of args.sources) {
        const label = escapeHtml(source.title || source.url || 'Untitled');
        let fromLabel = '';
        if (source.from) {
    ...
    await email.send({
      ...
      // Body is first-party markup built above; its AI/scraped values are escaped.
      trustedContent: true,
      data: {
        email: {
          preview: beehiivOk
