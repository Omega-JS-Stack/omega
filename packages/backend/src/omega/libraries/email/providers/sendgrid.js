...
// The one Omega instance, read at call time: this module first loads while
 ...
 *
 * Reads `omega.config.marketing.campaigns.providers.sendgrid.listId` (#425) —
 * populated by OMEGA's `campaigns/ensure/list.js` at brand-onboarding time, same
 * as how Beehiiv's `publicationId` works. No runtime API call, no fuzzy-match
 * fragility.
 *
 ...
 */
function getListId() {
  const listId = instance().config.marketing?.campaigns?.providers?.sendgrid?.listId;

  if (!listId) {
    console.warn(
      'SendGrid: marketing.campaigns.providers.sendgrid.listId is not set in config. '
...
async function buildFields(userDoc) {
  const values = resolveFieldValues(userDoc, instance().config);
