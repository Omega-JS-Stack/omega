...
TestProvider.prototype.request = async function (options) {
  ...
  assertAllowedEnvironment(self.omega);
...
function assertAllowedEnvironment(omega) {
  // The Omega instance's environment detection is the SSOT when in scope
  if (omega && typeof omega.isDevelopment === 'function' && typeof omega.isTesting === 'function') {
    if (omega.isDevelopment() || omega.isTesting()) {
      return;
    }
  ...
  // No Omega instance (pure unit tests) — allow only under explicit test/emulator signals
