...
const DISCOUNT_CODES = {
  ...
  'COMEBACK20': {
    ...
    duration: 'once',
    eligible: (user) => {
      return toUser(user).plan === 'basic' && user.subscription?.trial?.claimed === true;
    },
  },
  'MISSYOU25': {
    percent: 25,
    duration: 'once',
    eligible: (user) => {
      return toUser(user).everPaid && user.subscription?.status === 'cancelled';
    },
  },
  'TRYAGAIN10': {
    percent: 10,
...
function validate(code, user) {
  ...
  if (user && entry.eligible) {
    // A raw user doc or a User instance, alike
    if (!entry.eligible(user)) {
