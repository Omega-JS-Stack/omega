 ...
 */
module.exports = async ({ ctx, omega, context, change }) => {
 ...
 *
 * The handler used to call `Analytics(...).event()` with hyphenated
 ...
 * @returns {void}
 */
function trackNotification({ omega, ctx, event, token, data }) {
  try {
    deliverConversion({
      event: event,
