...
Module.prototype.main = function() {
  ...
  return new Promise(async function(resolve, reject) {
    return libraries.cors(req, res, async () => {
      ...
      assistant.log(`omega_api(${resolved.command}): Request (${geolocation.ip || 'unknown'} @ ${geolocation.country || '?'}, ${geolocation.region || '?'}, ${geolocation.city || '?'}) [${method} > ${strippedUrl}]`, JSON.stringify(assistant.request.data));
      assistant.log(`omega_api(${resolved.command}): Headers`, JSON.stringify(headers));
