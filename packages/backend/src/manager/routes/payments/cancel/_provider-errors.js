/**
 * Cancel-error classification — does a provider's failure mean the subscription
 * is ALREADY GONE?
 *
 * The cancel route may force-write a cancelled subscription for a suspended user
 * whose provider record no longer exists, so the user can re-subscribe. That
 * write must answer to the provider saying "no such subscription" and nothing
 * else: a network blip, an expired key, or a rate limit is transient, and
 * treating it as "already gone" hands out a cancellation the provider never
...
/**
 * Does this provider error mean the subscription no longer exists?
 *
 * @param {Error} e - The error a provider's cancel call threw
 * @returns {boolean}
 */
