...
module.exports = {
  async switchPlan({ resourceId, uid, subscription, product, frequency, ctx }) {
    ...
    const subscriptionObj = {
      ...
      // period — and the cancel processors read that equality (`trial.expires`
    ...
    ctx.log(`Test plan processor: wrote payments-webhooks/${eventId} (${eventType}) for sub=${resourceId}, uid=${uid}, product=${product.id}, frequency=${frequency}, trialEnd=${trialEnd}`);
  },
};
