/**
 * Test winback processor
 * Applies the save offer the way the real ones do: to the PROCESSOR, not to us.
 *
 * The other test processors write a payments-webhooks doc, because the thing
 ...
 *
 * So this processor does what Stripe's does to OUR data, which is nothing, and
 * says so. A local QA run through `_dev_cardProcessor=test` still proves the
...
module.exports = {
  ...
  async applyOffer({ resourceId, uid, discount, ctx }) {
    ...
    ctx.log(`Test winback processor: applied ${discount.code} (${off}, ${discount.duration}) to sub=${resourceId}, uid=${uid}`);
  },
};
