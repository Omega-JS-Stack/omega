  ...
  tests: [
    // --- Edge case tests ---
    {
      ...
      async run({ http, assert }) {
        ...
        const signupResponse = await http.as('referred-invalid').command('user:sign-up', {
          attribution: {
...

    {
      ...
      async run({ http, assert, state }) {
        // Call user:sign-up as the referred user with the new attribution format
        // This triggers the referral tracking logic
        // Use .as('referred') to authenticate as that specific user via privateKey
        const signupResponse = await http.as('referred').command('user:sign-up', {
          attribution: {
            affiliate: {
        ...
        assert.isSuccess(signupResponse, `user:sign-up should succeed: ${JSON.stringify(signupResponse, null, 2)}`);
...

    {
      ...
      async run({ http, assert, state }) {
        // Try to call user:sign-up again for the same user
...

    {
      ...
      async run({ firestore, assert, state, waitFor }) {
        ...
        // The user:sign-up command calls updateReferral() which updates the referrer
    ...
    // --- Auth rejection test (at end per convention) ---
    {
      ...
      async run({ http, assert }) {
        // Try to call user:sign-up without authentication
        const signupResponse = await http.as('none').command('user:sign-up', {
          attribution: {
