/**
 * Boot smoke — @omegajs/backend's framework self-test layer.
 *
 * Runs ONLY when `npx mgr test` is invoked from the @omegajs/backend repo (the
 ...
 *
 * This is @omegajs/backend's equivalent of BXM's `boot/extension-loads` and UJM's site-boot
  ...
  tests: [
    ...
    },
    {
      ...
      async run({ http, assert }) {
        ...
        // means the local @omegajs/backend (symlinked into the fixture) booted.
        const response = await http.get('backend-manager/test/health');
