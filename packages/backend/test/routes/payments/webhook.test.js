...
module.exports = {
  ...
  tests: [
...

    {
      ...
      auth: 'none',
      async run({ http, assert }) {
        const response = await http.as('none').post('backend-manager/payments/webhook?processor=stripe&key=wrong-key', {});
...

    {
      ...
      auth: 'none',
      async run({ http, assert }) {
        const response = await http.as('none').post(`backend-manager/payments/webhook?processor=unknown&key=${process.env.OMEGA_WEBHOOK_KEY}`, {
          id: 'evt_test_unknown',
          type: 'test.event',
          data: { object: {} },
        });
...

    {
      ...
      async run({ http, assert, firestore }) {
        ...
        const send = () => http.as('none').post(`backend-manager/payments/webhook?processor=test&key=${process.env.OMEGA_WEBHOOK_KEY}`, {
          id: eventId,
          type: 'customer.subscription.updated',
          data: {
            object: {
