/**
 * Test: POST /payments/webhook — the test processor's production guard
 ...
 * the webhook door. The `test` processor fabricates Stripe-shaped events, so it is
 * non-production only — the same rule the intent side enforces inside
 * `intent/processors/test.js`. The webhook processors receive only the raw req
 ...
 *
 * Run: npx omega test backend:routes/payments/webhook-test-processor
 */
...
module.exports = {
  description: 'Webhook test processor is non-production only',
  type: 'group',

  tests: [
    {
      name: 'test-processor-is-blocked-in-production',
      async run({ assert, Manager }) {
        ...
        assert.equal(sent.code, 403, `Test processor should be forbidden in production, got ${sent.code}`);
      },
    },
    ...
    {
      name: 'test-processor-is-allowed-outside-production',
      async run({ assert, Manager, ctx }) {
        ...
        assert.equal(sent.code, 200, `Test processor should be accepted outside production, got ${sent.code}`);
        assert.equal(sent.body.ignored, true, 'Unsupported event should be ignored');
      },
    },

    {
      name: 'the-key-still-gates-before-the-processor-guard',
      async run({ assert, Manager }) {
        const sent = await withProductionEnvironment(() => callWebhook({
          Manager,
          query: { processor: 'test', key: 'wrong-key' },
