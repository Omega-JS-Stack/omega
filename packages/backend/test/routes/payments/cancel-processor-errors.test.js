 ...
 *
 * A suspended subscriber whose processor record no longer exists gets their
 * subscription reset directly, so they can re-subscribe. That write used to fire
 * on ANY processor error: a network blip, an expired key, a rate limit — each
 ...
 *
 * Run: npx omega test backend:routes/payments/cancel-processor-errors
 */
...
const isAlreadyGone = require('../../../src/manager/routes/payments/cancel/_processor-errors.js');

// A subscriber on a real processor, old enough to clear the age guard.
function subscriber(Manager, { uid, status, processor }) {
  ...
  return buildUser(Manager, {
    ...
    subscription: {
      ...
      payment: {
        processor: processor,
        resourceId: 'sub_test_processor_error',
...
module.exports = {
  description: 'Payment cancel endpoint: processor error classification',
  type: 'group',
  timeout: 15000,
  ...
  tests: [
...

    {
      ...
      async run({ assert, Manager, firestore }) {
        ...
        const user = subscriber(Manager, { uid, status: 'suspended', processor: 'stripe' });
...

    {
      ...
      async run({ assert, Manager, firestore }) {
        ...
        const user = subscriber(Manager, { uid, status: 'active', processor: 'stripe' });
...

    {
      ...
      async run({ assert, Manager }) {
        const user = subscriber(Manager, { uid: '_test-cancel-neutral-message', status: 'active', processor: 'stripe' });

        const sent = await cancelWithoutStripeKey(Manager, user);

        assert.equal(sent.code, 500, `Expected the processor failure, got ${sent.code}`);
...

    {
      ...
      async run({ assert }) {
        ...
        const error = Stripe.errors.StripeInvalidRequestError.generate({
          ...
          statusCode: 404,
          message: "No such subscription: 'sub_test_processor_error'",
        });
...

    {
      ...
      auth: 'none',
      async run({ assert }) {
        // The shape libraries/payment/processors/chargebee.js throws
...

    {
      ...
      auth: 'none',
      async run({ assert }) {
        // The shape libraries/payment/processors/paypal.js throws — status in the message only
