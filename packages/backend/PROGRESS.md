...
## 📌 Active Task List
* [ ] One-off: HTTPS serve broken in somiibo-backend — stale foreign-CA certs + WM plain-http (2026-07-03)
  * [x] Diagnosis: (1) somiibo `.temp/certs` cert was issued by ANOTHER machine's mkcert CA (`ian@mac.lan` vs installed `ian@macbookpro.lan`) — browsers reject https://localhost:5002, only internal 5443 responds (the "hosted on 5443" symptom; 5443 itself is by design); (2) web-manager dev `getApiUrl()` was `http://localhost:5002` since forever — plain HTTP can't connect to the HTTPS-only proxy (BEM 5.7.0+), so frontend dev API calls fail regardless
  * [x] BEM fix: `_getHttpsCerts()` now validates existing certs (unexpired + chains to current `mkcert -CAROOT`) via `_checkCertProblem()`; wipes + regenerates on mismatch; no-mkcert fallback preserved. Standalone-verified: somiibo stale cert → "issued by a different CA", fresh cert → valid, real TLS handshake OK
  * [x] WM fix: dev `getApiUrl()` → `https://localhost:5002` (test updated; 71 passing). WM CHANGELOG [Unreleased]
  * [x] Deleted somiibo's stale `.temp/certs` (gitignored; regenerates trusted on next serve)
...
* [ ] One-off: Extended-mode test hardening — 5 provider-drift failures (2026-07-03)
  ...
  * [x] Fix 3: mailbox route tests dual-outcome (400-naming-the-check OR 200-with-status both prove the paid pipeline ran); carve-outs broadened from `'out of credits'` substring to ANY provider error (fail-open shape has no `status`); admin-only 400 now names the failing check (`Email validation failed (mailbox)`) in post.js + legacy twin
  * [x] Fix 3b: `test/functions/general/add-marketing-contact.js` was a stale pre-hardening copy of contact.js — mirrored all hardening (tests 1/5/7/9 + domain)
  * [x] Fix 4: validation.js — old `mailbox-fake-domain-fails` asserted the impossible (dns short-circuits before mailbox on dead domains); rewritten as `mailbox-not-called-when-dns-fails` + NEW `mailbox-nonexistent-mailbox-not-valid` (real-MX gmail address actually reaches the paid provider)
  * [x] Fix 5: `verify-edit` GitHub read-back polls via injected `waitFor` (60s/3s, timeout 90s) in BOTH twins (test/functions/admin/edit-post.js + test/routes/admin/post.js)
