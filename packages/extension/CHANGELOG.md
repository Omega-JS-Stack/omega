...
- **Ephemeral extension loading via the `chrome-devtools-extension` MCP upstream** — the extension-testing story under the new per-session isolated browser model. Launches **Chrome for Testing** (branded stable Chrome removed `--load-extension` ~v137 — re-verified ignored on 149; CfT still honors it — verified loaded on CfT 150) with the unpacked extension from `$BXM_EXTENSION_PATH` (→ `packaged/chromium/raw/`) pre-loaded into a per-session ephemeral profile. Rebuild loop = disable+enable the upstream. Documented in `docs/cdp-debugging.md` + CLAUDE.md.

### Changed
- **`docs/cdp-debugging.md` rewritten for the per-session isolated browser model** (mirrored across UJM/BEM/BXM/EM/WM). No manual Chrome launch, no ports, no shared `chrome-profiles/agent` — the load-unpacked-once-into-the-persistent-profile workflow is replaced by the upstream above.
