...
An Electron dev app is one running process: you ATTACH to it instead of launching a browser, through the `chrome-devtools-electron` MCP upstream (reads `OMEGA_CDP_PORT`, default 9222, expanded once at session start) or @omega.js/desktop's per-invocation `npx omega cdp`. See @omega.js/desktop's `docs/cdp-debugging.md`.
