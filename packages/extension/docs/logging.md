...
Everything that flows through stdout/stderr: `build.logger(...)` output, raw `console.log` calls, gulp task names, bundle/sass output, the works. ANSI color codes are stripped from the file (grep-friendly); the terminal continues to receive colored output unchanged.
