// Build-layer tests for build.getBuildErrorNotificationArgs() — the notifly
...
module.exports = defineCases({
  type: 'suite',
  layer: 'build',
  description: 'build module: build-error notification argv is shell-free',
  tests: [
    {
      name: 'returns an argv array, never a shell string',
      run: (ctx) => {
        const args = build.getBuildErrorNotificationArgs('Package', NASTY);
    ...
    },
    {
      ...
      run: (ctx) => {
        const args = build.getBuildErrorNotificationArgs('Bundle', 'boom');
    ...
    },
    {
      ...
      run: (ctx) => {
        ...
        try {
          build.reportNotificationFailure(result.error);
        } finally {
    ...
    },
    {
      ...
      run: (ctx) => {
        ...
        console.error = (...args) => errors.push(args.join(' '));
        try {
          build.reportNotificationFailure(failure);
        } finally {
          console.warn = origWarn;
          console.error = origError;
        }
