// Build-layer pin for the ads auto-bind (ads-system phase 4): every page
// surface (popup/options/sidepanel/page) wires lib/ads.js after
...
// @omega.js/client's own suite (client test/tests/ads.test.js).
...
const ADS_LIB = read('lib', 'ads.js');
const PACKAGE_TASK = read('gulp', 'tasks', 'package.js');

module.exports = {
  type: 'suite',
  layer: 'build',
  description: 'ads auto-bind — surface wiring + house-lane pin + build-JSON plumbing',
  tests: [
    {
      ...
      run: (ctx) => {
        for (const surface of SURFACES) {
          ...
          ctx.expect(source).toMatch(/import \{ wireAds \} from '\.\/lib\/ads\.js';/);
    ...
    },
    {
      ...
      run: (ctx) => {
        ctx.expect(ADS_LIB).toMatch(/omega\.ads\(\)\.mount\(\$el, \{ type: 'house' \}\);/);
        // The wiring marks bound hosts for observability (tests + debugging)
        ctx.expect(ADS_LIB).toMatch(/data-omega-ad-bound/);
        // Live binding: new [data-omega-ad] elements mount on arrival
        ctx.expect(ADS_LIB).toMatch(/new MutationObserver\(/);
      },
    },
    {
