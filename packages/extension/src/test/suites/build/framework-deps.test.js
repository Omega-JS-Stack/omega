...
// library @omega.js/extension declares by BARE specifier and it resolves from the
// FRAMEWORK's own installation — the framework's copy wins even when the consumer
// installed its own. The mechanism is the ORDER of `resolve.modules` in
// gulp/tasks/webpack.js (framework's node_modules before the project's), so these tests
// compile a real consumer entry with that exact module list and read the resolved module
// paths back out of `stats.toJson({ modules: true })`.
//
...
// consumer declares a conflicting version. Mirrors
// @omega.js/desktop's src/test/suites/build/framework-deps.test.js.
...
const { makeResolveModules } = require(path.join(FRAMEWORK_ROOT, 'src', 'gulp', 'tasks', 'webpack.js'));

// Framework-declared deps sampled by the first test: two bare packages plus a subpath
// import, all of them real `dependencies` of @omega.js/extension and all browser-safe
// (the extension bundles target the web).
const SAMPLE = ['json5', 'lodash', 'lodash/merge'];
...
// Compile `entry` with the module list the extension webpack task builds for this
// (projectRoot, frameworkRoot) pair, and return the module list.
function compile({ projectRoot, frameworkRoot }) {
  return new Promise((resolve, reject) => {
    webpack({
      ...
      resolve: { modules: makeResolveModules(projectRoot, frameworkRoot) },
    }, (err, stats) => {
...
function stageHoistedConsumer(specs) {
  ...
  const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'omega-extension-framework-deps-')));
  writePackage(root, 'consumer-extension', '1.0.0');
...
function stageConflictInstall() {
  const root          = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'omega-extension-framework-deps-dupe-')));
  const consumerNm    = path.join(root, 'node_modules');
  const frameworkRoot = path.join(consumerNm, '@omega.js', 'extension');

  writePackage(root, 'consumer-extension', '1.0.0', { dependencies: { 'dupe-pkg': '^1.0.0' } });
  ...
  writePackage(frameworkRoot, '@omega.js/extension', '1.0.0', {
...

    {
      ...
      run: async (ctx) => {
        ...
        try {
          ...
          ctx.expect(resolvedFor(modules, 'dupe-pkg')).toContain(path.join('extension', 'node_modules', 'dupe-pkg'));
