/**
 * @omega.js/extension's half of the shared dependency checks
...
/** See `@omega.js/devkit/target-dependencies` — bound to the extension's map + installer. */
function ensurePeerDependencies(input) {
  return shared.ensurePeerDependencies({ ...input, dependencyMap: DEPENDENCY_MAP, safeInstall });
}

/** See `@omega.js/devkit/target-dependencies` — bound to the extension's installer. */
