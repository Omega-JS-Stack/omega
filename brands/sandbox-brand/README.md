...
| `targets/website` | `packages/client` (@omega.js/client) | Minimal static site whose esbuild bundle embeds @omega.js/client, pointed at the emulator suite (environment=development auto-connects — zero flags, N5). Gets replaced by an `@omega.js/web` consumer in Phase 2 — the brand-monorepo slot and the e2e contract stay the same |
...
discovery (`targets/backend` + `targets/website`), website build + static serve,
