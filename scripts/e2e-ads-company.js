/**
 * Root `npm run test:ads` — ads step 6: the COMPANY-MODE PROOF
 * (plans/ads-system.md, sequencing 6: "Paperloom serves, a second in-repo
 ...
 * harness) and its Firestore `ads` collection is seeded with a small varied
 * inventory (tags, weights, one whitelist-scoped ad).
 *
 * Route-level proof (raw fetch against the hosting emulator):
 *   - eligible ad → the self-contained HTML unit (postMessage vocabulary,
 *     redirect link, port-preserving target origin, no external assets)
 *   - contextual scoring beats raw weight (tag-matched ad always wins)
 ...
 *   - whitelist scoping: an ad whitelisted to one sub-brand host serves
 ...
 * the client test setup), and the ads module's resolveSource() +
 * AdUnit.buildServeUrl() produce the URL that is then ACTUALLY fetched
...
if (process.env.OMEGA_SKIP_E2E === '1') {
  console.log('⏭ OMEGA_SKIP_E2E=1 — skipping ads company-mode e2e');
  process.exit(0);
}
...
const LOG_DIR = path.join(ROOT, '.temp', 'ads-e2e');
...
// ---------------------------------------------------------------------------
// Seeded inventory: varied tags + weights, one whitelist-scoped ad. Ads that
...
function startEmulator() {
  ...
  // --no-seed: personas are irrelevant to the ads proof (and skipping the
...
async function main() {
  ...
  try {
    ...
    await step('ads inventory seeds into the emulator Firestore', async () => {
      ...
      const existing = await db.collection('ads').get();
      for (const doc of existing.docs) {
        await doc.ref.delete();
      }
      for (const ad of ADS) {
        await db.doc(`ads/${ad.id}`).set(ad);
      }

      return `${ADS.length} ads (project ${projectId})`;
    });

    // -- 1. Paperloom serves (route level) --------------------------------

    await step('eligible ad serves as the self-contained HTML unit', async () => {
      const response = await fetch(`${base}/omega/ads/serve?parent=${CONSUMER_HOST}&tags=writing,notes`);
      ...
      assert.ok(body.includes('The Paperloom Desk Kit'), 'tag-matched ad title should render');
      ...
      assert.ok(body.includes('/omega/ads/redirect?id=paperloom-desk'), 'click link should ride the redirect route');
    ...
    await step('contextual scoring beats raw weight (5 of 5 rounds)', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const response = await fetch(`${base}/omega/ads/serve?parent=${CONSUMER_HOST}&tags=dev-news,ci`);
        const body = await response.text();
        assert.equal(response.status, 200, `serve should 200 (got ${response.status})`);
        assert.ok(body.includes('id=devnews-digest'), `round ${i + 1}: tag-matched ad should beat the heavier untargeted ad`);
      }
    });

    await step('no eligible inventory → 204 no-fill', async () => {
      // nofill.example: ads 1-2 blacklist it; the whitelisted ad fails closed
      const response = await fetch(`${base}/omega/ads/serve?parent=nofill.example`);
    ...
    await step('redirect 302s only to the stored link (+ UTM), fail-closed', async () => {
      const response = await fetch(
        `${base}/omega/ads/redirect?id=devnews-digest&parent=dailybuild.omegajs.dev&url=https://evil.example/hijack`,
        { redirect: 'manual' },
      );
      ...
      assert.equal(location.searchParams.get('utm_campaign'), 'devnews-digest', 'utm_campaign should be the ad id');

      const unknown = await fetch(`${base}/omega/ads/redirect?id=no-such-ad`, { redirect: 'manual' });
    ...
    await step('whitelisted ad serves ONLY on its listed sub-brand host', async () => {
      const listed = await fetch(`${base}/omega/ads/serve?parent=dailybuild.omegajs.dev&adId=dailybuild-exclusive`);
      const listedBody = await listed.text();
      assert.equal(listed.status, 200, `listed host should 200 (got ${listed.status})`);
      assert.ok(listedBody.includes('id=dailybuild-exclusive'), 'listed host should receive the whitelisted ad');

      // Another sub-brand pinning the same ad: the pin is ineligible — either
      // another ad serves or nothing does, but NEVER the whitelisted one
      const other = await fetch(`${base}/omega/ads/serve?parent=site.example&adId=dailybuild-exclusive`);
      const otherBody = other.status === 200 ? await other.text() : '';
      assert.ok(!otherBody.includes('id=dailybuild-exclusive'), 'a non-listed host must never receive the whitelisted ad');

      // And with everything else ineligible the whitelist fails CLOSED
      const closed = await fetch(`${base}/omega/ads/serve?parent=nofill.example&adId=dailybuild-exclusive`);
    ...
    await step('The Daily Build composes company-mode ads config (real @omega.js/config)', async () => {
    ...
    await step('real @omega.js/client resolves the company source to the parent stack', async () => {
      ...
      const source = Manager.ads().resolveSource();
    ...
    await step('consumer-built serve request lands on the parent inventory (the REAL hop)', async () => {
      const { AdUnit } = await import(pathToFileURL(path.join(CLIENT_SRC, 'modules', 'ads.js')).href);

      const source = Manager.ads().resolveSource();
      const unit = new AdUnit(Manager, {}, {
        source,
      ...
      assert.ok(serveUrl.startsWith(`http://127.0.0.1:${ports.hosting}/omega/ads/serve?`), `consumer URL should target the parent serve route (got ${serveUrl})`);
      ...
      assert.ok(body.includes('The Build Radar Digest'), "the consumer's tags should select the parent's tag-matched ad");
